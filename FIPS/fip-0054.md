...
Marks the current contract as deleted by setting the tombstone in state marking the current origin and nonce.
Transfers the balance out to the designated beneficiary.
This respects Ethereum semantics in that the contract continues to be "alive" until the end of the transaction.
...
- NonExistent (0) - The actor does not exist.
- System (1) - The actor is one of the built-in singleton actors (IDs 0-10, for now).
- Placeholder (2) - The actor is a placeholder.
- Account (3) - The actor is some type of account. Currently this includes EthAccount and Account actors, but it will include user-defined account types in the future as well.
- StorageProvider (4) - The actor is a storage provider.
- EVMContract (5) - The actor is FEVM contract.
- OtherTypes (6) - The actor exists, but is not one of the previously specified actor types.
