...
1. **Param validation**
   1. Assert that `SectorID` has no deals active
   2. Assert that either `SectorKeyCID` is non-null (which guarantees that this was once a CC sector) or that `SectorID` has no deals at all (active or otherwise)
   4. Assert proof size is not above limits
   5. Assert batch size bounds are not above limits
   6. Assert that sector's power is active. Fail if sector is in unproven, faulty or terminated states
