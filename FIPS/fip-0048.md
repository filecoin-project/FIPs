...
**NOTE**: For now, `Exec4` will only be callable by "blessed" address managers, the first of which will likely be the this will likely be the "Ethereum Address Manager (EAM)" (in NV18). Once users are able to deploy custom WebAssembly actors, this restriction should be relaxed (in a future FIP).
...
The embryo actor rejects all messages except simple value transfers (method 0) to ensure that funds can't be lost due to, e.g., a reorg. For example:
