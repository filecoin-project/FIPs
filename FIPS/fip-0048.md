...
**NOTE**: For now, f4 addresses may only be assigned by and in association with specific "blessed" address managers, the first of these will likely be the this will likely be the "Ethereum Address Manager (EAM)". Once users are able to deploy custom WebAssembly actors, this restriction should be relaxed (in a future FIP).
...
First, we'll define a special "placeholder" actor. This actor simply aborts with `USR_UNHANDLED_MESSAGE` (exit code 22) if invoked with any method number other than 0. The placeholder's implementation is simply:
...
2. Invoke the requested method on the placeholder, depositing any value transferred. For any method number other than 0, the placeholder will abort with `USR_UNHANDLED_MESSAGE` (see above) and the placeholder will be deleted.
...
### Rejecting Non-Zero Messages in the Placeholder Actor

The placeholder actor rejects all messages except simple value transfers (method 0) to ensure that funds can't be lost due to, e.g., a reorg. For example:
...
Because placeholder actors _reject_ all messages except simple value transfers, the message from U2 will _fail_ and no funds will be lost. If the placeholder _allowed_ messages other than simple value transfers, the funds would have been deposited but not credited to `some_address` and therefore would have been lost.
...
This highly depends on how code will be deployed to an [abstract account](https://github.com/filecoin-project/FIPs/discussions/388). It will likely involve a `CreateAccount` method on the ETH Address Manager, and will also likely involve the ability to automatically deploy an abstract account to a placeholder actor in some cases.
