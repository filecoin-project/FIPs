...
An alternative design we considered to reduce the storage between PreCommit and ProveCommit is NI-PoRep (see for example discussion https://github.com/filecoin-project/FIPs/discussions/603 ). NI-PoRep allows to get a 100% storage cost saving and has other benefits (for example, removing the need of PreCommitDeposit) but has a ~8.5x proving overhead that we consider prohibitive for now. 
...
In the current PoRep protocol if 4% or more of nodes in a layer are wrongly encoded by the SP, then the ProveCommit step, where 176 random nodes are checked, will fail with large probability (larger than $1-2^{10}$).
...
We can show that if `N_syn` is large enough, then this is not possible in practice. More in details: the probability that the number of correctly encoded nodes in the synthetic set is large or equal to $q$`N_syn` is given by the binomial probabilty:
