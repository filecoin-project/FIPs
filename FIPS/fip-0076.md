...
[FIP-0083](https://github.com/filecoin-project/FIPs/blob/master/FIPS/fip-0083.md) defines and proposes the implementation of a meaningful subset of
fire-and-forget externally observable events that will be emitted by the built-in Verified Registry, Storage Market and Storage Miner Actors.
These enable external agents to observe and track a specific subset of on-chain datacap allocation/claims,
deal lifecycle and sector lifecycle state transitions.

These events, especially the ones emitted by the Miner Actor for sector activation and sector updates will allow for better introspection of data
onboarded by the new Miner Actor methods proposed in this FIP as information pertaining to this data will not be available in the Market Actor state.
The gas costs for emitting these events is a trivial fraction of the overall gas costs for the methods that emit these events.
...
of the gas cost of verifying a single replica update.
