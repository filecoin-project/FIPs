...
This method takes in a sector number, generic serialized bytes encoding a sector location and the status of the sector, either `Live`, `Dead`, or `Faulty`.  It returns true if the correct sector status is passed.  Note that the sector location input paramter is specified to be a generic upgradeable byte slice format. The first implementation should us the standard tuple based cbor encoding of the `SectorLocation` type, i.e. `[i64, i64]`
...
To validate a `Faulty` sector the method loads the partition state at the input sector location. It returns true if and only if the `SectorNumber` is set in the partition's `faults` bitfield.  This will not be the case if the sector is 1) live and unfaulted or 2) terminated.
