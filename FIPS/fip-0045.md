...
An allocation is created by a verified client by transferring DataCap tokens 
to the verified registry (see below). 
...
Alternatively, any verified client can extend the term for a claim _beyond_ the initial maximum term by
spending new DataCap (see below).

The term maximum can be extended in this way up to the network policy maximum beyond the current epoch.
...
Instead, the `RemoveExpiredAllocations` method on the verified registry actor transfers any released
