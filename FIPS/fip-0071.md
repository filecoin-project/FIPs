...
|----------------------------|-----|-------------------------------------------------|
| `ipld_cbor_scan_per_field` | 35  | the fee charged per CBOR field read             |
| `ipld_cbor_scan_per_cid`   | 400 | the fee charged per CID read while parsing CBOR |
| `ipld_link_tracked`        | 300 | the fee charged per "reachable" CID tracked     |
| `ipld_link_checked`        | 300 | the fee charged per "reachable" CID checked     |
...
This FIP will _slightly_ increase the gas costs of reading/writing state (due to the IPLD link analysis). However, it is not expected that these costs will be significant: A single IPLD read costs at least 200k gas while link analysis is 35 gas per CBOR field and 700 gas per CID (both for reads and writes).

E.g., the Miner actor's root state has 16 CBOR fields (15 fields plus the object itself) and 7 links leading to ~5.5k gas. The current cost of opening the miner state-root is ~220k gas, so the _read_ cost increase is ~2.5%. Write cost overhead should be negligible as we already charge 3340gas/byte.
