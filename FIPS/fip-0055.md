...
Only select messages are allowed to carry delegated signatures. These are:

1. Messages to the Ethereum Address Manager (`f010`, calling its `CreateExternal` method, and carrying a validly CBOR-encoded byte array as its parameters (with no extra bytes)
2. `Send` messages with empty parameters
3. Messages with `MethodNum` equal to the EVM built-in actor's `InvokeContract` number, and carrying a validly CBOR-encoded byte array as its parameters (with no extra bytes)
...
| 1     | Recipient              | Filecoin address converted from the `EthereumTransaction#To`, or `f010` if nil as this denotes a contract deployment                        |
...
| 8     | Params                 | CBOR-serialization of the byte array `EthereumTransaction#Input`, which must not be empty if `EthereumTransaction#Recipient` is nil                                                                                                                 |
