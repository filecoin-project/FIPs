...
EIP-7939 specifies an Ethereum gas cost for `CLZ` (`5`, matching `MUL`). FEVM does not adopt Ethereum's fixed opcode gas schedule, so this FIP does not specify an Ethereum-style gas number for `CLZ`.

An implementation must not introduce a special syscall or pricelist gas charge for this operation. Execution is charged via FEVM's normal instrumented Wasm instruction accounting, so the observed cost depends on the Wasm-level execution cost of the `CLZ` implementation.
...
## Test Cases
An implementation of this FIP MUST include:
