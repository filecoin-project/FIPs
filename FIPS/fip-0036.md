...
## Introduction 
FIP-0036 has been passionately debated, adjusted, tweaked, tuned, and iterated upon over the course of the past couple of months. CryptoEconLab appreciates and encourages the detailed and thoughtful discussion our community continues to have surrounding this FIP, and, hope this level of informed and thoughtful discussion continues for future Filecoin Improvement Proposals (regardless of their ultimate inclusion in the protocol). As ZX put it [earlier](https://github.com/filecoin-project/FIPs/discussions/421#discussioncomment-3516156), "as all FIPs in Filecoin, it should never be us vs them. We are all part of the Filecoin community. It should be about how to make Filecoin better as a whole. This is the principle of CryptoEconLab and its network of teams and collaborators."

The updates to the FIP draft below reflect the work of CryptoEconLab and its network of contributors to propose community centered solutions, while also supporting that the health, stability, and prosperity of an economy as well as its contributing participants. The macroeconomic commentary [here](https://github.com/filecoin-project/FIPs/discussions/421#discussioncomment-3516156) can help provide some shared context for us admist navigating this FIP, its potential inclusion in upcoming upgrades, and, why many believe in strong net benefits inclusion in the upcoming network upgrade nv17, pending community support and acceptance. 

Thank you all for your continued insights and collaboration on this proposal.
...
- The minimum sector duration time will increase to 1 year and the maximum sector duration to 3.5 years.
- CEL and the community will monitor and look to increase the maximum duration to 5 years in a future FIP **if conditions merit**. 
...
Note that for the majority of sector qualities, PCD is less than initial pledge. We expect this relationship to continue to hold, as (1) the money supply is expands over time with long-term vesting and emissions, and (2) the expected reward per sector falls over time with growth in network capacity and decaying block reward. CEL has produced an accompanyng [tool](https://docs.google.com/spreadsheets/d/1mia7LyDSOh3lwAGhz7gTPUc65DQJbhHPyh013QpM4v4/edit#gid=0) with supporting documentation, allowing Storage Providers to estimate their expected Fil-on-Fil return given their projected failure rate to proveCommit sectors. Furthermore, since this amended policy has substantially reduced the max 

## Backwards Compatibility
This policy would apply at Sector Extension and Upgrade for existing sectors. 

### Extensions for FIL+ and CC Sectors 
A point raised by the SP community concerns the relative ability for CC sectors and FIL+ sectors to extend their sectors upon this policies implementation. In particular, some of these concerns centered on the ability for CC sectors to instantly extend to gain a potential 5x multiplier on their QAP. This updated FIP addreses these two concerns in two ways

1. We introduce a softer version of the policy, **capping the max sector duration commitment to 3.5 years instead of the previously proposed 5-year time-span**. 
...
### Rollout Shock
Initial iterations of this FIP suggested a potential max 10x multiplier on CC sectors of maximum duration. This could create a rollout shock if SPs race to extend their commitments and gain a further 10x multiplier. The new proposed iteration of the policy **caps the maximum CC multiplier to 3.5x**. This is, in part, due to the preference to *gradually* introduce changes to the network, monitor the impacts, and then strenghthen/adjust policy in subsequent upgrades. 
