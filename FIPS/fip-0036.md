...
- $SectorInitialConsensusPledge = 0.4 \cdot FILCirculatingSupply \cdot \frac{Sector_QAP}{max(BaselineTarget, NetworkQAP)}$
...
1. $advPower = advFILplus \cdot FILplusMultiplier \cdot durationMultiplier \cdot powerOnboarding \cdot FILplusPct$
2. $advPower =  0.5 \cdot 10 \cdot (2 \cdot 5) \cdot 50 \cdot 0.05$
3. $honestPower = 0.5 \cdot 10 \cdot (2 \cdot 5) \cdot 50 \cdot 0.05 + 0.5 \cdot 1 \cdot (2 \cdot 1) \cdot 50 \cdot 0.95$ 
4. $advPowerDailyPctGain = \frac{advPower}{18,000 + advPower + honestPower}$ 
