...
To assemble a premium distribution weighted by gas limit, dedupe the transactions in the tipset and sort by premium.
...
This method considers `maxqueueblks` and suggests MaxFeePerGas to users before they sign transactions.
This method SHOULD return the worst case base fee after `maxqueueblks` epochs.
$$ FeeCap(maxqueueblks) = BaseFee_{curr} * (1 + R)^{Max(1, maxqueueblks)} $$
