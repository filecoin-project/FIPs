...
The method `GasEstimateGasPremium` MAY recommending premiums by simulating the mempool according to `nblocksincl` and the remaining epoch time.
...
Currently, base fees are stuck near the minimum of 100 attoFIL, and premiums around 100 femtoFIL, so approximately 99.9% of gas fees are proposer revenue, while only 0.1% gets burned.
