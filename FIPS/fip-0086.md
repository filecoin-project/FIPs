...
8:    C ← {baseChain}
9:   step ← nil
10:   broadcast_msgs ← {} \* map with msgs sent by this participant per round and step


11:   while (step != DECIDE)  {
12:      if (round = 0)
13:       BEBroadcast <QUALITY, value, instance>; trigger (timeout)
14:       updateStep(round, QUALITY)
...
24:       updateStep(round, CONVERGE)
25:       collect a clean set M of  CONVERGE msgs from this round and instance
...
28:         C ← C ∪ {value}
29:       if (value ∈ C)
30:         proposal ← value \* we sway proposal if the value is incentive compatible (i.e., in C)
31:         evindence ← justification \* otherwise keep existing proposal and evidence

32:     reBroadcast <PREPARE, value, instance, round, evidence>; trigger(timeout) \* evidence is nil in round=0
...
34:     if (HasStrongQuorumValue AND StrongQuorumValue(M) = proposal) \* strong quorum of PREPAREs for local proposal
...
45:         reBroadcast <DECIDE, StrongQuorumValue(M), instance, evidence>  \* break loop, wait for other DECIDE messages
...
```
67: reBroadcast(msg): 
68:   broadcast_msgs[msg.round][msg.step] = msg
69:   BEBroadcast(msg)
70:   trigger(timeout_rebroadcast) 
71:   updateStep(msg.round, msg.step)
72:   upon timeout_rebroadcast expires:
73:     if step = msg.step AND msg.round = round AND msg.instance = instance: \* stuck, need to rebroadcast
74:       switch (msg.step) {
75:         case DECIDE:
76:           BEBroadcast(broadcast_msgs[0][DECIDE]) \* only rebroadcast DECIDE
77:           break;  // Exit, no cascading
78:         case COMMIT:
79:           BEBroadcast(broadcast_msgs[msg.round][COMMIT]) \* no break, cascade to broadcast PREPARE, CONVERGE
80:         case PREPARE:
81:           BEBroadcast(broadcast_msgs[msg.round][PREPARE]) \* no break, cascade to broadcast CONVERGE
82:         default: \* QUALITY or CONVERGE, which will never reach this point, but send CONVERGE here because of cascading effect of switch cases
83:           if msg.round > 0:
84:             BEBroadcast(broadcast_msgs[msg.round][CONVERGE])
85:             BEBroadcast(broadcast_msgs[msg.round-1][COMMIT])
86:             BEBroadcast(broadcast_msgs[msg.round-1][PREPARE])
87:             break;
88:       }
...
```
93: upon reception of a valid msg: \* assume validity implies same instance as currently running, different instances treated elsewhere (not scope of this doc)
94:   if msg.round < round OR (msg.round > round AND msg.step = COMMIT AND msg.value = 丄)  \* drop message
95:		  return
96:	  deliver(msg) 
97:   if  (msg' ← shouldJump(round, step) s.t. msg' != nil) 	\* one of the rest of conditions to be able to jump
98:     round ← msg'.round;
99:     timeout ← 2*Δ*backOffExponent**msg'.round
100:    timeout_rebroadcast ← max(timeout+1, timeout_rebroadcast)
101:    if msg'.evidence.step = PREPARE: \* either this or strong quorum of COMMITs for 丄
102:      C ← C ∪ {msg'.value}       \* add to candidate values if not there
103:      proposal ← msg'.value;     \* sway local proposal to possibly decided value
104:    evidence ← msg'.evidence   \* strong PREPARE quorum is inherited evidence for the value (it exists because the value might have been decided)
105:    go to line 21  \* start new round by jumping forward

106: shouldJump(round, step): 
107:   if step = QUALITY OR step = DECIDE: \* never jump on DECIDE or QUALITY
108:     return nil
109:   if (∃ msg' st. msg'.step = CONVERGE          \* there must be a CONVERGE for the new round
           AND msg'.round > round                                    \* round must be greater   
           AND ∃ M s.t. M is clean and valid and contains a weak quorum of PREPAREs for msg'.round) \* there must be a weak quorum of PREPAREs for the same round                                  
110:     return msg'
111:   return nil
```
...
The $\texttt{Valid}()$ predicate (referred to in lines 15, 25, 33, 41, 56, 58, 62, 93 and 109) is defined below.
```
