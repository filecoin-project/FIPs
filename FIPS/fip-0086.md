...
9:    C ← {baseChain}
10:   step ← nil
11:   broadcast_msgs ← {} \* map with msgs sent by this participant per round and step
...
46:     collect a clean set M of valid COMMIT messages from this round and instance
           until HasStrongQuorumValue(M)
           OR (timeout expires AND IsStrongQuorum(M))
47:     if (HasStrongQuorumValue(M))
48:      if (StrongQuorumValue(M) ≠ 丄)    \* decide
49:         evidence ← Aggregate(M)
50:         reBroadcast <DECIDE, StrongQuorumValue(M), instance, evidence>  \* break loop, wait for other DECIDE messages
51:      else \* no participant decided in this round
52:         evidence ← Aggregate(M) \* strong quorum of COMMITs for 丄 is evidence
53:     if (∃ m ∈ M: m.value ≠ 丄)) \* m.value was possibly decided by others
54:       C ← C ∪ {m.value}           \* add to candidate values if not there
55:       proposal ← m.value; \* sway local proposal to possibly decided value
56:       evidence ← m.evidence \* strong PREPARE quorum is inherited evidence
57:     round ← round + 1;
58:     timeout ← updateTimeout(timeout, round)
59:     timeout_rebroadcast ← max(timeout+1, timeout_rebroadcast)
60:   }  \*end while

61:   collect a clean set M of valid DECIDE messages
         until (HasStrongQuorumValue(M)) \* collect a strong quorum of decide outside the round loop
62:   return (StrongQuorumValue(M), Aggregate(M)) \* terminate the algorithm with a decision
```
```
\* decide anytime
63:   upon reception of a valid <DECIDE, instance, v, evidence> AND not decideSent
64:     reBroadcast <DECIDE, v, instance, evidence)
65:     go to line 61. 
```
...
```
66: mayHaveStrongQuorum(value, round, step, advPower): 
67:   M  ← { m | m.step = step AND m.round = round AND m is valid} /* clean set of messages
68:   M' ← { m | m ∈ M AND m.value = value } 
69:   if Power(M') + (1-Power(M)) < ⅔ - advPower  : \* value cannot have been decided 
70      return False
71:   return True
```
...
```
72: reBroadcast(msg): 
73:   broadcast_msgs[msg.round][msg.step] = msg
74:   BEBroadcast(msg)
75:   trigger(timeout_rebroadcast) 
76:   updateStep(msg.round, msg.step)
77:   upon timeout_rebroadcast expires:
78:     if step = msg.step AND msg.round = round AND msg.instance = instance: \* stuck, need to rebroadcast
79:       switch (msg.step) {
80:         case DECIDE:
81:           BEBroadcast(broadcast_msgs[0][DECIDE]) \* only rebroadcast DECIDE
82:           break;  // Exit, no cascading
83:         case COMMIT:
84:           BEBroadcast(broadcast_msgs[msg.round][COMMIT]) \* no break, cascade to broadcast PREPARE, CONVERGE
85:         case PREPARE:
86:           BEBroadcast(broadcast_msgs[msg.round][PREPARE]) \* no break, cascade to broadcast CONVERGE
87:         default: \* QUALITY or CONVERGE, which will never reach this point, but send CONVERGE here because of cascading effect of switch cases
88:           if msg.round > 0:
89:             BEBroadcast(broadcast_msgs[msg.round][CONVERGE])
90:             BEBroadcast(broadcast_msgs[msg.round-1][COMMIT])
91:             BEBroadcast(broadcast_msgs[msg.round-1][PREPARE])
92:             break;
93:       }
94:       update(timeout_rebroadcast); \* increase and trigger again timeout_rebroadcast 
95:       trigger(timeout_rebroadcast) 
```

The updateStep function simply updates the step and delivers locally all buffered messages for the new step and round:

```
96: updateStep(round, new_step): 
97:   step ← new_step
98:   receiveAll(receiver_queue.PopAll(round, step)) \* deliver all queued messages in the queue for this instance, round and step, and remove from queue
```

The conditions for skipping rounds and buffering messages are shown below:

```
99: upon reception of a valid msg: \* assume validity implies same instance as currently running, different instances treated elsewhere (not scope of this doc)
100:   if msg.round < round \* drop message
101:    return
102:   else if msg.step = DECIDE 
103:    deliver(DECIDE) \*DECIDE immediately delivered within an instance
104:  else if msg.step = step and msg.round = round
105:    deliver(msg) \* can be delivered as it is for this round and step
106:  else if msg.round > round
107:    receiver_queue.Add(msg) 
108:    receiver_queue.Trim() \* selectively drop messages if bound met. See text below.
109:  if  (msg' ← shouldJump(round, step) s.t. msg' != nil) 	\* one of the rest of conditions to be able to jump
110:    round ← msg'.round;
111:    timeout ← 2*Δ*backOffExponent**msg'.round
112:    timeout_rebroadcast ← max(timeout+1, timeout_rebroadcast)
113:    if msg'.evidence.step = PREPARE: \* either this or strong quorum of COMMITs for 丄
114:      C ← C ∪ {msg'.value}       \* add to candidate values if not there
115:      proposal ← msg'.value;     \* sway local proposal to possibly decided value
116:    evidence ← msg'.evidence   \* strong PREPARE quorum is inherited evidence for the value (it exists because the value might have been decided)
117:    go to line 22  \* start new round by jumping forward

118: shouldJump(round, step): 
119:   if step = QUALITY OR step = DECIDE: \* never jump on DECIDE or QUALITY
120:     return nil
121:   if (∃ msg' ∈ receiver_queue st. msg'.step = CONVERGE          \* there must be a CONVERGE for the new round
           AND msg'.round > round                                    \* round must be greater   
           AND ∃ M ⊆ receiver_queue s.t. M is clean and valid and contains a weak quorum of PREPAREs for msg'.round) \* there must be a weak quorum of PREPAREs for the same round                                  
122:       return msg'
123:   return nil
```

The function receiver_queue.Trim() contains a queue that keeps all messages in the current round, all validated QUALITY messages (as QUALITY from future instances may be valid but the participant may not have locally the power table that must be used to validate it), and all valid messages carrying a justification, but can drop all other messages (i.e. it drops COMMITs for 丄 not in the current round). QUALITY messages in the queue can be removed once the QUALITY phase is executed. Also, once a valid DECIDE message is received, all other messages in the queue for this instance can be removed. 
...
The $\texttt{Valid}()$ predicate (referred to in lines 16, 26, 38, 46, 61, 63, 67, 99 and 121) is defined below.
```
...
The synchronization of participants is performed in the call to $\texttt{updateTimeout(timeout, round)}$ (line 58), and works as follows:
