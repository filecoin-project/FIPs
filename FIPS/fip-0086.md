...
```
72: reBroadcast(msg): 
73:   latest_broadcast[msg.step] = msg
74:   BEBroadcast(msg)
...
79:       switch (msg.step) {
80:         case DECIDE:
81:           BEBroadcast(latest_broadcast[DECIDE]) \* only rebroadcast DECIDE
82:           break;  // Exit, no cascading
83:         case COMMIT:
84:           BEBroadcast(latest_broadcast[COMMIT]) \* no break, cascade to broadcast PREPARE, CONVERGE
85:         case PREPARE:
86:           BEBroadcast(latest_broadcast[PREPARE]) \* no break, cascade to broadcast CONVERGE
...
88:           if msg.round > 0:
89:             BEBroadcast(latest_broadcast[CONVERGE])
90:             break;
91:       }
92:       update(timeout_rebroadcast); \* increase and trigger again timeout_rebroadcast 
93:       trigger(timeout_rebroadcast) 
```

The updateStep function simply updates the step and delivers locally all buffered messages for the new step and round:

```
94: updateStep(round, new_step): 
95:   step ← new_step
96:   receiveAll(receiver_queue.PopAll(round, step)) \* deliver all queued messages in the queue for this instance, round and step, and remove from queue
```

The conditions for skipping rounds and buffering messages are shown below:

```
97: upon reception of a valid msg: \* assume validity implies same instance as currently running, different instances treated elsewhere (not scope of this doc)
98:   if msg.round < round \* drop message
99:    return
100:   else if msg.step = DECIDE 
101:    deliver(DECIDE) \*DECIDE immediately delivered within an instance
102:  else if msg.step = step and msg.round = round
103:    deliver(msg) \* can be delivered as it is for this round and step
104:  else if msg.round > round
105:    receiver_queue.Add(msg) 
106:    receiver_queue.Trim() \* selectively drop messages if bound met. See text below.
107:  if  (msg' ← shouldJump(round, step, timeout) s.t. msg' != nil) 	\* one of the rest of conditions to be able to jump
108:    round ← msg.round;
109:    timeout ← 2*Δ*backOffExponent**msg.round
110:    timeout_rebroadcast ← max(timeout+1, timeout_rebroadcast)
111:    if msg'.evidence.step = PREPARE: \* either this or strong quorum of COMMITs for 丄
112:      C ← C ∪ {msg'.value}       \* add to candidate values if not there
113:      proposal ← msg'.value;     \* sway local proposal to possibly decided value
114:    evidence ← msg'.evidence   \* strong PREPARE quorum is inherited evidence for the value (it exists because the value might have been decided)
115:    go to line 22  \* start new round by jumping forward

116: shouldJump(round, step, timeout): 
117:   if step = QUALITY OR step = DECIDE: \* never jump on DECIDE or QUALITY
118:     return nil
119:   if (∃ msg' ∈ receiver_queue st. msg'.step = CONVERGE          \* there must be a CONVERGE for the new round
           AND msg'.round > round)                                   \* round must be greater     
120:     if ∃ M ⊆ receiver_queue s.t. M is clean and valid and contains a weak quorum of PREPAREs for msg'.round
121:       return msg'
122:     if timeout expired AND NOT canTerminateStep(round, step)
123:       return msg'
124:   return nil
```
...
The $\texttt{Valid}()$ predicate (referred to in lines 16, 26, 38, 46, 61, 63, 67, 97 and 120) is defined below.
```
