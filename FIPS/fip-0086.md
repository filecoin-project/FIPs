...
6':   receiver_queue ←newQueue(size: 7*n) \* bounded queue, where n is the number of participants.
7:    C ← {baseChain}
7':   step ← nil
8':   latest_broadcast ← {} \* map with latest msgs sent by this participant per step
...
23:			  if (evidence is a strong quorum of PREPAREs AND mayHaveStrongQuorum(value, r-1, COMMIT, 1/3)):
24:				  C ← C ∪ {value}
25:			  if (value ∈ C)
...
30:     reBroadcast <PREPARE, value, instance, round, evidence>; trigger(timeout) \* evidence is nil in round=0
...
38:     reBroadcast <COMMIT, value, instance, round, evidence>; trigger(timeout)
...
42:       reBroadcast <DECIDE, StrongQuorumValue(M), instance, evidence>  \* break loop, wait for other DECIDE messages
44:     if (∃ m ∈ M: m.value ≠ 丄 s.t. mayHaveStrongQuorum(m.value, r, COMMIT, 1/3)) \* m.value was possibly decided by others
...
51':    timeout_rebroadcast ← max(timeout+1, timeout_rebroadcast)
52:   }  \*end while
...
57:	  reBroadcast <DECIDE, v, instance, evidence)
58:	  go to line 53. 
```
...
64:     latest_broadcast[msg.step] = msg
65:	BEBroadcast(msg)
66:  	trigger(timeout_rebroadcast) 
67:     updateStep(msg.round, msg.step)
68:	upon timeout_rebroadcast expires:
69:		If step = msg.step AND msg.round = round AND msg.instance = instance \* stuck, need to rebroadcast
70: 			switch (msg.step) {
71:     				case DECIDE:
72:         					BEBroadcast(latest_broadcast[DECIDE]) \* only rebroadcast DECIDE
73:         					break;  // Exit, no cascading
74:     				case COMMIT:
75:        					BEBroadcast(latest_broadcast[COMMIT]) \* no break, cascade to broadcast PREPARE, CONVERGE, QUALITY also
76:     				case PREPARE:
77:						BEBroadcast(latest_broadcast[PREPARE]) \* no break, cascade to broadcast CONVERGE, QUALITY also
78:     				default: \* QUALITY or CONVERGE, which will never reach this point, but send CONVERGE here because of cascading effect of switch cases
79: 					if msg.round > 0:
80:						BEBroadcast(latest_broadcast[CONVERGE])
81:         				break;
82: 			}
83:			update(timeout_rebroadcast); \* increase and trigger again timeout_rebroadcast 
...
86:	step ← new_step
90: 	receiveAll(receiver_queue.PopAll(round, step)) \* deliver all queued messages in the queue for this instance, round and step, and remove from queue
```

We show below the conditions for skipping rounds, and buffering messages. 

```
91: upon reception of a valid msg: \* assume validity implies same instance as currently running, different instances treated elsewhere (not scope of this doc)
...
101:       	if  (msg' ← shouldJump(round, step, timeout) s.t. msg' != nil) 	\* one of the rest of conditions to be able to jump
102:			round ← msg.round;
103:   			timeout ← 2*Δ*2**msg.round
104:			timeout_rebroadcast ← max(timeout+1, timeout_rebroadcast)
105:			if msg'.evidence.step = PREPARE: \* either this or strong quorum of COMMITs for 丄
106:	 			C ← C ∪ {msg'.value} 		\* add to candidate values if not there
107:				proposal ← msg'.value; 		\* sway local proposal to possibly decided value
108:			evidence ← msg'.evidence	\* strong PREPARE quorum is inherited evidence for the value (it exists because the value might have been decided)
109:			go to line 16  \* start new round by jumping forward

110: func shouldJump(round, step, timeout): 
111: 	if step = QUALITY OR step = DECIDE: \* never jump on DECIDE or QUALITY
112: 		return nil
114:     if (∃ msg' ∈ receiver_queue st. msg'.step = CONVERGE		\* there must be a CONVERGE for the new round
	     AND msg'.round > round)					\* round must be greater	
115: 		if ∃ M ⊆ receiver_queue s.t. M is clean and valid and contains a weak quorum of PREPAREs for msg'.round
116:			return msg'
117:		if timeout expired AND NOT canTerminateStep(round, step)
118:			return msg'
119:	return nil
```

The function canTerminateStep(round, step) returns True if the step contains the conditions to terminate in the round (lines 19, 30, 38, for CONVERGE, PREPARE, COMMIT, respectively), and False otherwise. 
