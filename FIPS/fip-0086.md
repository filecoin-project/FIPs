...
```
The conditions for skipping rounds and buffering messages are shown below (below, retain_future_rounds is an implementation config paameter):
...
101:    if (msg.round > round + retain_future_rounds) AND msg.step = COMMIT AND msg.value = 丄  
...
The $\texttt{Valid}()$ predicate (referred to in lines 15, 25, 33, 41, 56, 58, 62, 100 and 109) is defined below.
```
