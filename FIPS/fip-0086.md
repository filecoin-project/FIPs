...
For Termination, we assume a classical partial (eventually) synchronous model in which there is $\Delta$-synchrony after an unknown Global Stabilization Time (GST).
...
>
> **Progress.**
> Let c be the heaviest common prefix of the inputs of all honest participants in instance $i$ and let i be the first instance that all honest participants start after GST. Then, if an honest participant finalizes $(i',c',\ast)$, $c$ is a prefix of $c'$ with probability equal to the proportion of honest participants for all $i'>i$.
> 
>
...
>
> `F3.ECupdate (int i, chain c)`: Participant $p$ updates F3 instance $i$ with chain $c$, which its EC instance locally delivered (relevant to EC compatibility)
...
24:				  C ← C ∪ {value}
25:			  if (value ∈ C)
26:				  proposal ←value \* we sway proposal if the value is EC compatible (i.e., in C)
...
Valid(m):                                      | For a message m to be valid,
  ...
  if m.step IN {QUALITY}                       |   If the step never requires evidence
      if m.evidence != nil:                    |   (QUALITY), evidence should not be present.
        return False                           |
      if len(m.value) > 100:		       |   Proposal's head must be at most 99 epochs
...
* Participants set their timeout for the current round to $Δ*2^{r}$ where $r$ is the round number ($r=0$ for the first round).
