...
23:       BEBroadcast <CONVERGE, value, instance, round, evidence, ticket>; trigger(timeout)
24:       collect a clean set M of valid CONVERGE msgs from this round and instance
          until timeout expires
25:       prepareReadyToSend ← False
26:		  while (not prepareReadyToSend){ 
27:			  value, evidence ← LowestTicketProposal(M)	\* leader election
28:			  if (evidence is a strong quorum of PREPAREs AND mayHaveStrongQuorum(value, r-1, COMMIT, 1/3)):
29:				  C ← C ∪ {value}
30:			  if (value ∈ C)
31:				  proposal ←value \* we sway proposal if the value is incentive compatible (i.e., in C)
32:				  prepareReadyToSend ← True    \* Exit loop 
33:			  else 
34:				  M = {m ∈ M | m.value != value AND m.evidence.value != evidence.value} \* Update M for next iteration } 

35:     reBroadcast <PREPARE, value, instance, round, evidence>; trigger(timeout) \* evidence is nil in round=0
36:	collect a clean set M of valid PREPARE messages from this round and instance
...
37:	if (HasStrongQuorumValue AND StrongQuorumValue(M) = proposal) \* strong quorum of PREPAREs for local proposal
38:       value ← proposal \* vote for deciding proposal (COMMIT)
39:       evidence ← Aggregate(M) \* strong quorum of PREPAREs is evidence
40:     else
41:       value ← 丄 \* vote for not deciding in this round
42:       evidence ← nil

43:     reBroadcast <COMMIT, value, instance, round, evidence>; trigger(timeout)
44:     collect a clean set M of valid COMMIT messages from this round and instance
        until (HasStrongQuorumValue(M) AND StrongQuorumValue(M) ≠ 丄)
	  OR (NOT mayHaveStrongQuorum(value, round, step, 0) for all value ≠ 丄)
          OR (timeout expires AND Power(M)>2/3)
45:     if (HasStrongQuorumValue(M) AND StrongQuorumValue(M) ≠ 丄)    \* decide
46:       evidence ← Aggregate(M)
47:       reBroadcast <DECIDE, StrongQuorumValue(M), instance, evidence>  \* break loop, wait for other DECIDE messages
48:     if (∃ m ∈ M: m.value ≠ 丄 s.t. mayHaveStrongQuorum(m.value, r, COMMIT, 1/3)) \* m.value was possibly decided by others
...
52:     else \* no participant decided in this round
53:       evidence ← Aggregate(M) \* strong quorum of COMMITs for 丄 is evidence
54:     round ← round + 1;
55:     timeout ← updateTimeout(timeout, round)
56:     timeout_rebroadcast ← max(timeout+1, timeout_rebroadcast)
57:   }  \*end while

58:   collect a clean set M of valid DECIDE messages
      until (HasStrongQuorumValue(M)) \* collect a strong quorum of decide outside the round loop
59:   return (StrongQuorumValue(M), Aggregate(M)) \* terminate the algorithm with a decision
```
```
\* decide anytime
60: upon reception of a valid <DECIDE, instance, v, evidence> AND not decideSent
61:	  reBroadcast <DECIDE, v, instance, evidence)
62:	  go to line 58. 
```
...
```
63: mayHaveStrongQuorum(value, round, step, advPower): 
64:	M  ← { m | m.step = step AND m.round = round AND m is valid} /* clean set of messages
65:     M' ← { m | m ∈ M AND m.value = value } 
66:     f Power(M') + (1-Power(M)) < ⅔ - advPower  : \* value cannot have been decided 
67:		return False
68:     return True
```
...
```
69: func reBroadcast(msg): 
70:     latest_broadcast[msg.step] = msg
71:	BEBroadcast(msg)
72:  	trigger(timeout_rebroadcast) 
73:     updateStep(msg.round, msg.step)
74:	upon timeout_rebroadcast expires:
75:		If step = msg.step AND msg.round = round AND msg.instance = instance \* stuck, need to rebroadcast
76: 			switch (msg.step) {
77:     				case DECIDE:
78:         					BEBroadcast(latest_broadcast[DECIDE]) \* only rebroadcast DECIDE
79:         					break;  // Exit, no cascading
80:     				case COMMIT:
81:        					BEBroadcast(latest_broadcast[COMMIT]) \* no break, cascade to broadcast PREPARE, CONVERGE, QUALITY also
82:     				case PREPARE:
83:						BEBroadcast(latest_broadcast[PREPARE]) \* no break, cascade to broadcast CONVERGE, QUALITY also
84:     				default: \* QUALITY or CONVERGE, which will never reach this point, but send CONVERGE here because of cascading effect of switch cases
85: 					if msg.round > 0:
86:						BEBroadcast(latest_broadcast[CONVERGE])
87:         				break;
88: 			}
...
```
91: func updateStep(round, new_step): 
92:	step ← new_step
93: 	receiveAll(receiver_queue.PopAll(round, step)) \* deliver all queued messages in the queue for this instance, round and step, and remove from queue
```

The conditions for skipping rounds and buffering messages are shown below:

```
94: upon reception of a valid msg: \* assume validity implies same instance as currently running, different instances treated elsewhere (not scope of this doc)
95: 	if msg.round < round \* drop message
96:		return
97:	else if msg.step = DECIDE 
98:		deliver(DECIDE) \*DECIDE immediately delivered within an instance
99:	else if msg.step = step and msg.round = round
100:		deliver(msg) \* can be delivered as it is for this round and step
101:	else if msg.round > round
102:		receiver_queue.Add(msg) 
103: 		receiver_queue.Trim() \* selectively drop messages if bound met. See text below.
104:       	if  (msg' ← shouldJump(round, step, timeout) s.t. msg' != nil) 	\* one of the rest of conditions to be able to jump
105:			round ← msg.round;
106:   			timeout ← 2*Δ*2**msg.round
...
111:			evidence ← msg'.evidence	\* strong PREPARE quorum is inherited evidence for the value (it exists because the value might have been decided)
112:			go to line 21  \* start new round by jumping forward

113: func shouldJump(round, step, timeout): 
114: 	if step = QUALITY OR step = DECIDE: \* never jump on DECIDE or QUALITY
115: 		return nil
116:     if (∃ msg' ∈ receiver_queue st. msg'.step = CONVERGE		\* there must be a CONVERGE for the new round
	     AND msg'.round > round)					\* round must be greater	
117: 		if ∃ M ⊆ receiver_queue s.t. M is clean and valid and contains a weak quorum of PREPAREs for msg'.round
118:			return msg'
119:		if timeout expired AND NOT canTerminateStep(round, step)
120:			return msg'
121:	return nil
```

The function canTerminateStep(round, step) returns True if the step contains the conditions to terminate in the round (lines 24, 36, 44, for CONVERGE, PREPARE, COMMIT, respectively), and False otherwise. 
...
The $\texttt{Valid}()$ predicate (referred to in lines 15, 24, 36, 44, 58, 60, 64, 94 and 117) is defined below.
```
...
The synchronization of participants is performed in the call to $\texttt{updateTimeout(timeout, round)}$ (line 55), and works as follows:
