...
This proposal resolves that tension by **shifting the signal** from upfront data and demand classification to demonstrated service delivery and demand response. Rather than trying to predict which data/service is valuable and when demand is real to allocate block reward, we set aside a fraction of the reward in the new Service Reward Actor. The actor quarterly deposits the proven allocation to Service Orchestrators that deploy those funds to generate on-chain deal revenue, and burns the remainder.
