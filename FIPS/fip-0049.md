...
The flags introduced herein are only used as a client hint, in this case to
signal indexing intent. Clients should honor the request, and construct indices
that facilitate fast lookup.

In the future, this flag and others may lead to consensus-relevant outcomes,
...
struct MessageReceipt {
    ...
    gas_used: i64,
    events: Cid, // new field; root of AMT<StampedEvent>
}
```
...
- An out of gas causes all events emitted by the call stack hitherto to be
  discarded.
...
committed to the chain state with no further structural changes needed.
...
This syscall will create a `StampedEvent` object, set the ActorID of the emitter
in the corresponding field, and store it in the events accumulator.
