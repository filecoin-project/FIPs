...
1. Validate structural correctness of received object, optionally deserializing
...
2. Validate the correctness of entry values, by checking they are well-formed
   DAG-CBOR that type restrictions are met (currently, they can only be CBOR
...
committed to the chain state via this new `events_root` field.

Note that just like the FVM does not write receipts to the blockstore, neither
...
1. Actor event validation fee, applied as soon as we enter the syscall,
   anticipating the cost of event validation.
2. Actor event processing fee, applied when we accept the event for processing.

The formulae are as follows:

``` golang
// The byte length of the supplied event.
input_len := len(actor_event)

// The amortized validation cost per byte.
p_event_validation_per_byte := TODO

// Validation fee
// 
// Covers:
// - Cost of validating the incoming `ActorEvent` object.
validation_fee := (p_event_validation_per_byte * input_len) +
                  (p_memcpy_gas_per_byte * input_len) +
                  (p_memretention_gas_per_byte * input_len)

// The estimated size of the data structure to be committed on chain,
// identical to the supplied event decorated with the emitter field +
// some DAG-CBOR typing metadata.
stamped_event_estsize := input_len + 12

// Cost of byte indexed, equated to the storage cost per byte.
...
// - Anticipated cost of copying, storing, and flushing the `StampedEvent` to the client, currently
//   equated to an individual ipld::block_link operation with hash Blake2b-256.
processing_fee := (p_memcpy_gas_per_byte * stamped_event_estsize) +
                  (p_memretention_gas_per_byte * stamped_event_estsize) +
                  (p_index_cost_per_byte * total_indexed_bytes) +
                  ipld_block_link_cost(hash="Blake2b-256", len=stamped_event_estsize)
```
...
We make conservative assumptions to simplify gas calculations and perform
point-in-time charging covering anticipated future work. Concretely, events are
buffered in an in-memory accumulator and not released into the blockstore until
the call stack concludes successfully. Events are released as part of an AMT flush, which means  as part of flushing the AMT. However,  
