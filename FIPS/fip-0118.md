...
4.  **Weights and shares updates.** `QuarterlyGateCheck` compares the
    ...
    the admitted lists, the pricing parameters,
    `REGISTRATION_CUTOFF`) through the ordinary
...
| `REGISTRATION_CUTOFF` | Late-claim guard: a registration in the last cutoff of a quarter applies from the boundary (Section 2.2) | Governance parameter | Parameter events | 7 days | Next quarter boundary | Registrations only; reassignments and releases keep their own application rules (Section 2.2) |
...
divided among Orchestrators. In particular,
  ...
  - The SRA holds the registry (the (payer, operator) to Orchestrator
    ...
    2.2) go through the same lifecycle but are not stored: their
    write only emits an event, since they parameterize the
    off-chain measurement, not an on-chain computation. Read-only views expose the registry, the bound
  ...
  - Its methods:
      ...
      - **`RegisterPairs(pairs)`.** Callable by an admitted
        ...
        parameter write, with the same event record and
        next-quarter application (Section 2.3); the pre-activation
        write installs the initial value, listed in the recap table
