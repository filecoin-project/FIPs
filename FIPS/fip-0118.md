...
2.  **Verification window mechanics.**
    ...
    5.  Determinism: `FPV_i(Q)` is fully determined by (i) settlement
        ...
        **Application rules.** A registration applies from its call
        epoch and also covers the pair's unbound past inside the
        same quarter, back to the later of the quarter's first epoch
...
4.  **Weights and shares updates.** `QuarterlyGateCheck` compares the
    ...
    are deployed before activation (Backwards Compatibility), and SRA
    Governance installs the initial state (the initial Orchestrator,
    the admitted lists, the pricing parameters) through the ordinary
    methods before the activation epoch, each emitting its event; a
    change binding before the activation epoch applies from quarter
    1's first epoch. Client pairs need no pre-activation install: a
...
divided among Orchestrators. In particular,
  ...
  - Its methods:
      ...
      - **`RegisterPairs(pairs)`.** Callable by an admitted
        ...
        parameters: its value is maintained in the governance
        repository (proposed start: 7 days) and a change applies
        from the next quarter boundary.
...
changes to leader election or finality.
  ...
  - **Governance over the registry (SRA capture).** A captured SRA
    ...
    published rationale, prospective for any binding older than its
    current quarter (`inherit`, Section 3.2), and never reaching an
    already-posted quarter (Section 2.2). One
