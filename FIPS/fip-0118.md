...
2.  **Split the block reward into weighted streams.** At each
    ...
    reward BR by weight:
      ...
      - the residual `w0 x BR`, with `w0 = 1 − w2 − w1`, to the burn
        actor.
...
directly. As in those systems, the weights are not static: they can
...
3)  **Stream list**: the ordered list of Stream records f02 holds in
    ...
    registry list is going to change infrequently so more than a few
    entries will make it storage-wasteful).
...
reduced by the non-consensus portion (`1 − w1`). SPs who serve clients may
offset this through deal payments; SPs who do not bear a permanent
reduction, which is the intended outcome.
