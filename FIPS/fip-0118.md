    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Qualifying print.* A fee-auction claim, on an admitted
        ...
        deviates from the last qualifying print by no more than
        `PRICE_BAND`. Lots below `MIN_LOT`, or priced outside the
        band, still auction and burn normally; they just do not
        price. `MIN_LOT` exists because the competitive-bidding
      ...
      - *Parameters.* `PRICE_BAND` (maximum deviation from the last
        ...
        indexer, and every verifier, must apply (the values in force
        at an epoch are those of the most recent parameter event at
        or before it). Keeping no on-chain state reduces friction and
...
divided among Orchestrators. In particular,
  ...
  - The SRA holds the registry (the (payer, operator) to Orchestrator
    ...
    SRA state, updated by SRA Governance under
    `SRA_CANCEL_HOLD` like any registry change (Section 4.2); the
