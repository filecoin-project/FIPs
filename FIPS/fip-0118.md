...
**1.2 New sectors, existing sectors and SnapDeals.**
  ...
  - Introduce `ExtendSectorExpiration3` that allows existing CC sectors
    to be extended and gain 9× QAP under the same pledge rule.
...
1.  **Volume reporting**. An off-chain indexer computes `FPV_i(Q)` (see
    ...
    aggregated value is their componentwise sum,
...
| `total_burn_minted` | New counter: cumulative burn (w0 residual plus rounding dust). |
...
| `next_transition_epoch` | New: the earliest effective epoch over the pending writes; recomputed on queue, apply and cancel; a sentinel when the queue is empty. The per-block path compares this one integer and touches `streams_root` only when a write is due. |
| `swa_timelock_epochs` | New: the SWA hold in epochs (`SWA_TIMELOCK`), set per network by the migration and never written afterwards. Mainnet: 7 days; test networks may set it shorter. |
...
The `streams_root` block holds:

```
streams[]:        [ id, WeightRecord, Distribution ]    // Section 2.4.1
tombstones[]:     [ id, payable[] ]                     // Section 2.4.6
pending_writes[]: [ id, op, payload, effective_epoch ]  // Section 2.4.7
```

Each element is specified in the section noted: `WeightRecord` and the
two Distribution forms in Section 2.4.1, tombstones (a removed
stream's outstanding liabilities) in Section 2.4.6, the queue in
Section 2.4.7. A pending write is a deferred method call: op names
one of the five queued SWA methods (Section 2.4.9) and payload
carries that call's parameters, executed verbatim at `effective_epoch`.
The wallet-keyed tables (shares, payable, `claimed_period`, a
tombstone's payable) are logical maps stored as arrays of
(ID address, value) pairs: the value is a u64 fixed-point share in
shares and a `TokenAmount` in the payable and `claimed_period` tables.
Each `EXPLICIT` stream owns its own tables, so the same wallet may
appear in more than one stream without collision, and a `Claim`
rewrites only its own stream's rows.
...
SetShares(id, new_map):     // designated writer only; never queued
    ...
    require Σ new_map shares == DENOM
...
exactly one. A recipient that does not resolve to an ID address
...
their position; an all-zero batch is a benign no-op, so keepers get
idempotent re-runs for free. Claims work mid-period, and funds can
only go to the wallets the share map (or tombstone) names, so a third
party settling on a recipient's behalf changes who pays the gas, not
...
Every SWA write queues in f02 and applies only after the hold. This is
the L1 enforcement of the Section 4 objection window: `SWA_TIMELOCK`,
stored per network as `swa_timelock_epochs` (7 days on mainnet), binds
even if the SWA itself is compromised or maliciously upgraded.
`SetShares` and `Claim` never queue.

The queue is a set of slots keyed (id, op): one pending write per
operation per stream. Queueing into an occupied slot is rejected, so
revising a pending write is cancel plus requeue and always restarts
the hold; no path changes a pending payload while keeping its
effective epoch. `effective_epoch` = queue epoch + `swa_timelock_epochs`;
`RegisterStream` may schedule later via its `activation_epoch`, the hold
being a floor. Due writes apply in effective-epoch order (id as
tiebreak) at the start of `AwardBlockReward` and of every mutating
method, so no message executes against stale configuration.
`CancelPending` is itself a mutating method, so a due write applies
before a same-epoch cancel: the objection window is exactly
`[queue, effective)`.
...
step. Canceling an empty slot is a benign no-op, since both Safes may
relay the same objection; the cancellation event fires only on
removal.
...
checks: per record, `0 <= floor <= cap <= 1`; and the projected weights
...
The per-block sum check therefore becomes an assertion: no reachable
sequence of accepted writes can violate it, and there is no fallback
weight vector. The SWA should still pre-validate before submitting,
and the queued records are public during the hold so anyone can
recompute the breakpoints, but f02's queue-time check is
authoritative.
...
`RegisterStream` validates that the stream count stays below
`MAX_STREAMS`, the recipient count within `MAX_RECIPIENTS`,
...
The slopes cancel over Q1 so nothing burns during bootstrap, and w2
hits its cap exactly at the Q1 boundary, exiting the bootstrap ramp
...
receives issuance, not how much, so the total keeps its slot:
  ...
  - **`total_burn_minted`** and **`total_service_minted`** are stored
    counters for the burn and service contributions, which are
    otherwise hard to recover independently: each needs the full
    weight and reward history.
...
divided among Orchestrators. In particular,
  ...
  - The SRA holds the registry (the (payer, operator) to Orchestrator
    ...
    conversion finalization, Sections 2.2 and 2.3), which the SWA’s
    gate reads.
  ...
  - Its methods:
      ...
      - **`PostVolume(Q, fpv)`.** Callable by an admitted, non-frozen
        ...
        verification window (Section 2.2), during which SRA Governance
      ...
      - **`FinalizeConversion(Q)`.** Permissionless and idempotent;
        ...
        final USD figures (Section 2.2). Every bound period is priced
...
changes to leader election or finality.
  ...
  - **FPV reporting: liveness and verification.** The gate’s integrity
    ...
    recomputation and answered through audit, freeze, or removal, since
  ...
  - **FIL pricing manipulation.** FIL-denominated volume is priced by
    ...
    recomputable and correctable before binding (Section 2.2). In the
  ...
  - **Compromised Orchestrator wallet.** An Orchestrator address holds
    ...
    dispute-correctable. A key compromise cannot redirect the split, but
