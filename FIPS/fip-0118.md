...
**1.2 New sectors, existing sectors and SnapDeals.**
  ...
  - SnapDeals (`ProveReplicaUpdates3`) remain available: snapping data
  ...
  - `UpgradeSectorQuality` (method 37) is a batch method that upgrades
    existing sectors to 10× QAP under the same pledge rule. Any active
    sector (not faulty, unproven, or terminated)
    ...
    Each declaration must select at least one sector. A supplied
    `new_expiration` must be later than the current epoch and every
    selected sector's current expiration, and remain within the existing
    extension and seal-proof lifetime bounds.
  ...
  - Upgrading a sector scales its recorded daily fee in proportion to
    the QAP change. A pre-FIP-0100 sector with no recorded fee gets a
    new fee at the full 10× level. SnapDeals follows the same rule.
    Extension alone leaves an existing fee unchanged and computes one
    only if the recorded fee is zero.
  ...
  - Expected behavior: most empty legacy CC sectors will be upgraded
    ...
    metrics). If every 1× byte were snapped or upgraded to 10×,
...
**1.3 Extensions: carry forward the recorded ratio**
  ...
  - Each declaration must select at least one sector across `sectors`
    and `sectors_with_claims`. The new expiration must be later than the
    current epoch and every selected sector's current expiration, and
    remain within the existing extension and seal-proof lifetime bounds.
  ...
  - Effect: extension carries the recorded multiplier forward. Integer
    division can slightly reduce QAP for an unflagged legacy sector.
    This does not rescale its existing daily fee. A sector carrying
    `FULL_QA_POWER` remains at 10× independently of its weights.
...
**1.4 Weight accounting cleanup.**
  ...
  - Piece activation records total piece space rather than separate
    verified and unverified amounts. Prove-commit validates CommD
    separately; both prove-commit and replica update use total piece
    space to compute the recorded weight.
...
2.  **Stream Weights Actor (SWA): the split AMONG streams.** It
    ...
    that stream. f02 itself enforces the hard guardrails (`Σ_{i>=1} w_i <= 1`,
...
Section 3.2). Quarter Q is `[Start(Q), Start(Q+1))`, with
`Start(Q) = ACTIVATION_EPOCH + (Q-1) * EPOCHS_PER_QUARTER`: it starts at
`Start(Q)` inclusive and stops at the epoch before `Start(Q+1)`, so
`Start(1)` is the activation epoch.
...
*Figure 2: one quarter, end to end. Steps 1 to 6 run from `Start(Q+1)`. Durations shown (3-day posting, 7-day verification) are the initial mainnet values, fixed at SRA deployment (Section 3.2); `SWA_TIMELOCK` is 7 days (Section 2.4). `QuarterlyGateCheck` (4) and `SubmitShares` (5) can run in either order.*
...
2.  **Verification window mechanics.**
    ...
    1.  Posting: each Orchestrator posts `FPV_i(Q)` to the SRA during
        the posting period [`Start(Q+1)`, `Start(Q+1)` + `POST_PERIOD`)
        (`Start` as defined above).

    2.  Verification: the verification window is a fixed phase of the
        quarter timeline, [`Start(Q+1)` + `POST_PERIOD`, `Start(Q+1)` +
        `POST_PERIOD` + `VERIFICATION_WINDOW`), and opens when the posting period
    ...
    4.  `SubmitShares` and `QuarterlyGateCheck` become callable from
        `Start(Q+1) + POST_PERIOD + VERIFICATION_WINDOW`, the first epoch
        after the window, and read only bound values, with
    ...
    5.  Determinism: `FPV_i(Q)` is fully determined by (i) settlement
        ...
        **Application rules.** An Orchestrator claims a payer by
        ...
        applies from its execution epoch and also covers the pair's
        unbound past inside the same quarter, back to the later of
        the quarter's first epoch and the epoch the pair last became
        unbound (back-cover), so a payer's settlements count from the quarter's
        start however long its Orchestrator takes to register the
        pair. A registration made in
        `[Start(Q+1) - REGISTRATION_CUTOFF, Start(Q+1))` applies from
        `Start(Q+1)` and covers nothing of quarter Q, so no claim can
        reach a quarter that closes before
        governance had the full cutoff to contest it (Section 3.2).
        A reassignment applies per its `inherit` argument (Section
        3.2); a release, by removal or by `CancelBinding`, applies at
        the release epoch. The
        admitted lists and the pricing parameters apply from the first
        epoch of the next quarter, so no quarter
        already being measured changes scope under the Orchestrators
        (Section 3.2). Within an epoch, registry writes and
        settlements are ordered as in *Print ordering* (Section 2.3).
        "Applies from the execution epoch" means from the write's
        position in that order: the write governs every settlement
        strictly after it, including later settlements in the same
        epoch, and none before it. Back-cover and `inherit = true` reach back in
        the same order. "At the release epoch" has the same meaning.
        "From `Start(Q+1)`" and "from the first epoch of the next
        quarter" mean from that epoch's first settlement. A verifier
        reconstructs the binding timeline
        from the registry events every change emits.
        ...
        The table below collects the application rules for the
        indexer; the text above is normative.

        | Registry write | Caller | Application epoch | The pair's settlements before the application epoch, in execution order, count for | The pair's settlements from the application epoch onward, in execution order, count for |
        | --- | --- | --- | --- | --- |
        | `RegisterPairs` | The Orchestrator, for itself | The execution epoch | The registrant, back to the later of the quarter's first epoch and the epoch the pair last became unbound; earlier than that, unchanged | The registrant |
        | `RegisterPairs` in `[Start(Q+1) - REGISTRATION_CUTOFF, Start(Q+1))` | Same | `Start(Q+1)` | Nobody: nothing of quarter Q | The registrant |
        | `CancelBinding` | The bound Orchestrator, for itself | The execution epoch | The releasing Orchestrator | Nobody, until a new registration, whose back-cover reaches the release epoch |
        | `ReassignBinding`, `inherit = false` | Both Registry multisigs | The execution epoch | The previous holder | The new holder |
        | `ReassignBinding`, `inherit = true` | Both Registry multisigs | The displaced registration's execution epoch, under that epoch's back-cover and cutoff, if its application epoch is in the current quarter or later; otherwise the execution epoch | Unchanged (whoever held the pair before the displaced registration) | The new holder; the displaced claimant keeps nothing |
        | `ReassignBinding` of a pair unbound at execution | Both Registry multisigs | The execution epoch, whatever `inherit` says | Nobody | The new holder |
        | `RemoveOrchestrator` | Both Registry multisigs | The execution epoch, for every pair the removed Orchestrator held | Nobody: the removed Orchestrator's FPV is excluded from the quarter | Nobody, until a new registration, whose back-cover reaches the release epoch |
        | `AddOrchestrator` | Both Registry multisigs | The execution epoch; the Orchestrator's registrations then follow the `RegisterPairs` rows | n/a | n/a |
...
4.  **Weights and shares updates.** `QuarterlyGateCheck` compares the
    ...
    immediately (Section 2.4.4). Orchestrators removed in quarter Q
    (that is, before epoch `Start(Q+1)`) are excluded from both
    computations: they are omitted from the
    submitted share map (shares in the map must be positive, Section
    2.4.4) and their FPV does not enter `AggregatedFPV(Q)`.
...
**Payment period.** A quarter's numbers pay out after the quarter ends, on a different set of days than the days measured. Quarter Q's gate write (Figure 2, step 4) queues in f02 under `SWA_TIMELOCK`, so the stepped w2 pays from `Start(Q+1) + POST_PERIOD + VERIFICATION_WINDOW + SWA_TIMELOCK` at the earliest, about 17 days into quarter Q+1 with the initial values (Section 3.2); the share map (step 5) binds without a hold, from about 10 days in. Both are floors: `QuarterlyGateCheck` and `SubmitShares` are permissionless cranks, nothing on chain executes them, and the timelock starts at the gate check's execution epoch, so a late crank delays the effect by the same amount. It reads the same bound values (step 3), so the amounts do not change. During the delay f02 burns the difference between the ceiling and the standing w2 every epoch (Section 2.1).

![w2 stepping behind its ramping ceiling, each step about 17 days after a quarter end, with the burn weight oscillating in the gap](../resources/fip-0118/payment-period.png)

*Figure 5: w2 against its ceiling on the all-pass path, drawn with zero crank delay (every permissionless call at its first callable epoch). The ceiling 1 - w1 ramps about 5pp per quarter; w2 steps 5pp once per quarter, about 17 days after each quarter end (posting, verification, timelock), so the burn w0 oscillates between about 0.94% just after a step and about 5.93% just before the next. The first gate check, marked on the axis, is callable from `Start(Q3)` + 10 days. The dashed staircase steps at each quarter end with no payment lag, for comparison. A late crank only widens the gap.*
...
#### 2.3 FPV definitions
    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Parameters.* `PRICE_BAND` (maximum deviation from the last
        ...
        applies from the first epoch of the next quarter, the same
        rule as the
  ...
  - **Filecoin Pay contract versions.** "Filecoin Pay" refers to the
    ...
    required. A list update applies from the first epoch of the next
    quarter
...
| --- | --- | --- | --- | --- | --- | --- |
| `PRICE_BAND` | Thin-auction guard: maximum deviation of a print's rate from the band reference | Governance parameter | Parameter events | 30% | From the first epoch of the next quarter | The first prints have no reference: *Band seeding* above |
...
| `MIN_LOT_FLOOR` | Input to `MIN_LOT(Q)` | Governance parameter (atto-USD) | Parameter events | $0.50 (5 * 10^17 atto-USD) | From the first epoch of the next quarter | None |
| `MIN_LOT_ALPHA` | Input to `MIN_LOT(Q)` | Governance parameter (rational: numerator and denominator) | Parameter events | 1/400 | From the first epoch of the next quarter | Retuned when needed |
...
| Admitted lists | Scope prints and FPV: a print is a claim on an admitted contract, of an admitted stablecoin | Governance parameter, event-only | `SetAdmittedLists` events | Installed pre-activation (Section 2.2) | From the first epoch of the next quarter | None |
...
| `REGISTRATION_CUTOFF` | Late-claim guard: a registration in the last cutoff of a quarter applies from `Start(Q+1)` (Section 2.2) | Governance parameter | Parameter events | 20,160 epochs (7 days) | From the first epoch of the next quarter | None |
...
1)  **Stream**: a named, weighted portion of the block reward:
    ...
    3.  **Distribution**: the stream's recipients and how the stream
        ...
        1.  **shares**: the stored wallet-to-share map, f099 rows
            removed, installed by `SetShares` or `ReplaceAddress`;
...
2)  **Stream list**: the ordered list of Stream records f02 holds in
    ...
    tables change through its designated writer (`SetShares`,
    `ReplaceAddress`) and through `Claim`.
...
Encoding follows Filecoin's canonical IPLD conventions:
  ...
  - `accrued`, `streams`, and `tombstones` have unique ascending stream
    IDs; recipient tables and share maps have unique recipients
    ascending by numeric actor ID; `pending_writes_queue` is ordered by
    effective epoch, preserving admission order at equal epochs.
...
struct State {
    ...
    swa_timelock_epochs: ChainEpoch,        // 12 new; migration-set only
    swa_actor: Address,                     // 13 new; migration-set only
    accrued: Vec<StreamAccrual>,            // 14 new
...
AwardBlockReward(miner, penalty, gas_reward, win_count):
    ...
    for each active stream s:                     // in list order
        ...
        if s.distribution is IMPLICIT:
            ...
            burn += portion - accrue
    burn += BR - allocated
    if the resulting stream and accrual state is invalid:
        no_award()
    total_minted_reward += BR; total_burn_minted += burn
    pay miner_reward + gas_reward to winning miner; penalties as today
    send(f099, burn + fold_dust)         // plus a miner reward that failed to send

no_award():
    pay gas_reward and apply penalty as today; return without state change
```

The due writes are applied first, on the state the award would store.
`no_award` discards that state, so the writes stay queued and apply at
the next award or mutating method; only a full award commits them.
...
Accounting is valid when `accrued` holds one row per live `EXPLICIT`
stream and no other, every accrual is non-negative, every `payable` and
`claimed_period` amount is positive, and no wallet's `claimed_period`
exceeds its live portion of the current period (Section 2.4.5).
...
Once the award commits the due folds, their fold dust is no longer owed
to recipients but has not yet been burned, so it is excluded from `BR`
and sent to f099 with the residual. Burning the exact residual conserves
`BR` atto-exactly despite per-stream floors. Gas rewards are unchanged
and the allocation path calls no FEVM contract or recipient. A failed
send to the miner burns that miner's reward. A failed burn send is
logged and the award stands, its value staying with f02 for a later
award to spend or burn.

##### 2.4.4 SetShares and the period fold

A **period** in f02 is the interval between two folds on a stream. A
map install (`SetShares`, `ReplaceAddress`), a writer change and a
removal (Section 2.4.6) each fold first. f02 knows nothing of quarters
and imposes no cadence, so the writer may install at any time.
The quarterly rhythm is SRA discipline: the SRA reaches `SetShares`
from `SubmitShares` alone, once per quarter after the verification
window, and reaches `ReplaceAddress` from `RemoveOrchestrator` and
`ReplaceWallet` (Section 3.2). Admission takes payment effect only at a
later `SubmitShares`, never through a mid-quarter push. Installing a new
map first closes the current period under the old one, and that fold is
what makes immediate binding safe: earnings materialize under the shares
...
New recipients in `RegisterStream`, `SetShares` and `ReplaceAddress`
must not be payment channels. The paych `Collect` method deletes the actor
and as a share recipient this would strand its rewards and accounting rows.
This restriction applies only to recipient admission, not writers, existing
rows, claims or awards. f099 is allowed.

`ReplaceAddress(id, old, new)` changes one recipient's future share.
Only the stream's designated writer may call it, and it takes effect
immediately. Both addresses must resolve to existing actors. `old` must
be in the stored share map; `new` must not already be in it. Setting
`new` to f099 removes the row and burns its share until the next map
change. Addresses may be supplied in any form that resolves.

The call folds the period before changing the row. Accrued balances
stay with the wallets that earned them; the replacement starts a new
current-period tally. The same installation checks as `SetShares`
apply, including the row reservations in Section 2.4.6.
...
`SetShares` and `ReplaceAddress` reject an install if the union of
post-fold `payable` recipients and the new share map exceeds the
...
map install while a removal is pending recomputes every reservation from
the post-fold `payable ∪ shares` and rejects the change if the aggregate
exceeds the cap. Application relies on that reservation rather than
checking again: a tombstone holds at most the rows it covered. A removed
...
upgraded. `SetShares`, `ReplaceAddress`, `Claim` and `CancelPending` never
queue.
...
projected weights
must satisfy `Σ_{i>=1} w_i(e) <= 1` at every epoch from the write's
effective epoch onward, so the burn residual w0 stays non-negative.
Each weight is a clamped linear function of the epoch
(`ComputeWeight`), so the sum is piecewise linear: between breakpoints
it is a straight line, and a line at or below 1 at both ends stays at
or below 1 in between. `Σ <= 1`
therefore need only hold at the write's effective epoch and, from
there on, at:
...
cancellation, `SetShares` and `ReplaceAddress` remain available;
due writes apply only when their result passes full validation. Every
write due before a repair is discarded in effective-epoch order. New
writes are rejected except a timelocked `SetWeightRecords` batch whose
projected application restores valid records and a valid future schedule
without stranding an admitted write. Awards pay gas rewards only until
that repair lands (Section 2.4.3).
...
migration recovers it. That upgrade is an emergency response, not a
normal FIP cycle: awards pay only the gas reward until it lands
(Section 2.4.3), and a gas-only interval is acceptable only because
it stays short.

Inconsistent accounting aborts every explicit method; `AwardBlockReward`
pays gas only, as Section 2.4.3 specifies. These are fail-closed
responses to inconsistent or corrupted state, not alternative allocation
modes.
...
| `ReplaceAddress(id, old_address, new_address)` | the stream's designated writer | immediate |
...
writer before resolving recipients; `ReplaceAddress` uses the same
writer authorization (Section 2.4.4). `Claim` checks `MAX_RECIPIENTS`
...
Before storing changes, each mutating method validates the affected
streams block or accrual rows. Failed validation aborts an explicit
method with an illegal-state exit and no state change. On the award
path it triggers `no_award` instead (Section 2.4.3).
...
w2 hits its cap exactly at `Start(Q2)`, exiting the bootstrap ramp
...
recipient to ID form before persistence, and rejects a bootstrap
recipient that is missing or a payment channel in the input state tree.
  ...
  - **State.** The SWA stores:
    ...
    methods:
      ...
      - **`QuarterlyGateCheck()`.** The mechanism entry point;
        ...
        the value it computes. `QuarterlyGateCheck` emits an event
        recording the checked quarter and the result whether or not the
        gate steps, so tooling can pinpoint when each check ran without
...
Service Orchestrators. It is a step function that can only increase once per
quarter, at the earliest about 17 days after the measured quarter ends
(the payment period, Section 2.2), and only if on-chain Filecoin Pay
Volume clears a target. The weight w2 evolves differently in Q1 (bootstrap) vs.
Q2 onward:
...
reaches `W2_BASE` (10%) exactly at `Start(Q2)`, where the
...
automatically from `Start(Q2)`: w1 keeps ramping down while w2
...
becomes callable immediately after. Two steps landing this way is
each gate being cleared by its own quarter's volume, not catch-up: the
latch counts quarters checked, not calendar time. They do not land in
quick succession, though: gate writes share one slot (Section 2.4.7),
so a passing check whose predecessor's write is still in its hold
reverts and is re-attempted once that hold expires. Consecutive late
passes therefore land at least one `SWA_TIMELOCK` apart, each still
computed from its own quarter's volume.
...
the gate is re-attemptable later (on this FIP's schedule the rejection
can occur only while a previous gate write is still in its hold, or
after a discretionary write, applied or pending, has taken part of the
ceiling `1 − w1`: a change to w1's schedule or a new stream). A
conflicting pending write blocks the gate even if it takes effect
later: admission cannot invalidate a queued write (Section 2.4.7). w2 takes values only on the 5pp grid
...
`SetGateParams`, and the action MUST be sequenced so that no
`QuarterlyGateCheck` applies between the two effective epochs; the
check is latched to once per quarter, so queueing the pair after the
current quarter's check leaves no check that can apply inside the
window. The SWA enforces this when the two holds overlap:
`QuarterlyGateCheck` reverts while a `SetWeightRecords` write is in
its hold and while a `SetGateParams` write is held in the SWA-internal
timelock, so no check computes a step from a counter that a queued
retune is about to replace; the delayed check runs once the write
takes effect or is cancelled (late cranks self-heal, above). When the
two holds do not overlap, as when the second multisig approves the
`SetGateParams` only after the `SetWeightRecords` hold has expired, a
check can execute between them, so the duty remains for that case.
Record and counter otherwise diverge: a
...
themselves are as defined in Section 2.2 and maintained in the SRA.
...
divided among Orchestrators. In particular,
  ...
  - **State.** The SRA stores:
    ...
    |---|---|---|
    | The registry: the `(payer, operator)` to Orchestrator bindings and each Orchestrator's payout wallet | `RegisterPairs`, `ReassignBinding`, `CancelBinding`, `AddOrchestrator`, `RemoveOrchestrator`, `ReplaceWallet` | The initial Orchestrator, seated by the activation migration; no bindings |
    ...
    | `EPOCHS_PER_QUARTER` | SRA code upgrade | 262,974 epochs (floor(EPOCHS_IN_YEAR / 4) with EPOCHS_IN_YEAR = 31,556,925 s / 30 s as in builtin-actors; 91.31 days) |
    ...
    Governance like any registry change, applying from the first
    epoch of the next quarter
    (Section 2.3), but are not stored: each write
  ...
  - **Removal semantics.** The registry knows two states, admitted
    ...
    already accrued: from the epoch it binds, `ReplaceAddress` removes the
    Orchestrator's map entry by naming `f099` (Section 2.4.4). Its slice
    burns until the next `SubmitShares` installs a fresh map. Everything
  ...
  - Its methods:
      ...
      - **`RegisterPairs(pairs)`.** Callable by an admitted
        ...
        2.3).
        A registration binds at once; it applies from the execution
        epoch with back-cover, and inside the last `REGISTRATION_CUTOFF`
        of a quarter from `Start(Q+1)`, per the application rules of
        Section 2.2. The cutoff parameterizes the
      ...
      - **`PostVolume(Q, fpv)`.** Callable by an admitted
        ...
        per quarter. The call rejects any `Q` other than the quarter
        whose posting period is open, the quarter that just ended.
      ...
      - **`SubmitShares()`.** Permissionless. Operates on the latest
        ...
        removed in quarter Q are excluded (Section 2.2). The new map
        redistributes the share burned after a removal (Section 2.4.4),
        so this call is the only moment the survivors gain from a removal.
      ...
      - **`AddOrchestrator(orch, wallet)`.** Both Registry multisigs.
        Binds at once and applies at once: the Orchestrator may
        register pairs and post volume from the admission epoch, its
        registrations applying under the rules of Section 2.2. An
        Orchestrator admitted in quarter Q therefore posts `FPV_i(Q)`
        in Q's posting period and enters the share map that quarter's
        `SubmitShares` installs, about ten days into Q+1.

      - **`RemoveOrchestrator(orch)`.** Both Registry multisigs.
        Callable only while no ended quarter awaits its share map:
        from `Start(Q+1)` until quarter Q's `SubmitShares` has run (the
        posting period, the verification window, and any crank delay
        after them), the call reverts, so every removal happens inside
        a quarter and is excluded from it (Section 2.2). The guard
        ...
        message; the cost of that interval is in Security
        Considerations.
        Applies the removal semantics above through an immediate
        `ReplaceAddress` naming `f099`, which drops the entry and leaves
        every other share untouched (Section 2.4.4). Redistribution to the
      ...
      - **`ReplaceWallet(old, new)`.** Both Registry
        ...
        an immediate `ReplaceAddress` call (Section 2.4.4). The replace
        is prospective: the Orchestrator identity does not change and
        accrued balances are not moved. The public request and the remedy
        of Section 4.3 protect against misuse; a mistaken replace is undone
        by another `ReplaceWallet`.
      ...
      - **`ReassignBinding(pair, orch, inherit)`.** Both Registry
        ...
        span it served. With `inherit = true`, used for a claim
        adjudicated wrongful, the reassignment adopts the displaced
        registration's execution epoch. The new binding's application
        is computed from that epoch under the application rules of
        Section 2.2 (back-cover, the cutoff). The rightful holder
        therefore gets the span it would have had by registering then,
        including any span the wrongful claim left unattributed, and
        the wrongful claimant keeps nothing. `inherit` is honored only when the displaced
        ...
        a span it already served. A reassignment of a pair that is
        unbound at execution applies from the execution epoch,
        whatever the argument says: nothing is displaced. A closed
        quarter is out of reach
      ...
      - **`CancelBinding(pair)`.** Callable by the bound Orchestrator,
        for itself; no governance call. Releases the pair: the call
        reverts unless the caller holds the binding, the binding is
        deleted and the release applies at the release epoch, its
        execution epoch, like a release by removal (Section 2.2), so
        the Orchestrator keeps the span it served and any admitted
        Orchestrator may claim the pair again through `RegisterPairs`,
        with back-cover reaching no further back than the release. Use
        it for a cooperative exit: a client relationship that ended,
        or a client moving to another Orchestrator. A contested
        binding is resolved by SRA Governance through
        `ReassignBinding` or removal. The event
        carries the released Orchestrator's identity, since the
        registry no longer does.

      - **`SetAdmittedLists(stablecoins, filecoin_pay_contracts)`.**
        Both Registry multisigs. Binds at once but applies from the
        first epoch of the next quarter, so no quarter already
        being measured
      ...
      - **`SetPricingParams(min_lot_floor, min_lot_alpha,
        ...
        the first epoch of the next quarter, the same rule as
        the admitted
...
pending upgrade and its execution MUST be observable on chain. f02
enforces the hold on the SWA's pending f02 writes (`SWA_TIMELOCK`,
Section 4.3). The SWA itself keeps the SWA-internal timelock on
gate-parameter changes and code upgrades (Section 3.1). The
...
accepted FIP (except the last):
  ...
  - alter a stream: rewrite its Weight record (`SetWeightRecords`) or
    re-route its Distribution; a w2 record change carries the pairing
    and sequencing duty of Section 3.1.1;
...
logic at all? This does not fit, for two reasons.
  ...
  - First, *co-signing and objection are different problems*: nesting
    ...
    capability from how the approval
...
### Parameter Choices and Calibration
  ...
  - **Burn is not a failure signal.** A gap is open even when every
    ...
    once per quarter, at the earliest about 17 days after the measured
    quarter closes (the payment period, Section 2.2). On the all-pass path
...
The implementation test suite MUST cover at least:
  ...
  - extension declaration bounds, carried QAP ratios, and an unchanged
    existing daily fee when integer rounding moves QAP;
  ...
  - `ComputeWeight` clamping and rounding, the Q1 w1/w2 bootstrap,
    ...
    (quarter ordering, late cranks, two consecutive late passes with the
    second check reverting while the first step's write is pending and
    succeeding after that hold, and atomic revert on f02 rejection),
  ...
  - stream registration, distribution and recipient-address changes,
    share-map validation and immediate binding, payable-cap rejection and claim relief, period
  ...
  - monotone issuance counters, the derived explicit-stream reserve,
    unchanged `FilMined`, reserve-shortfall and accounting-failure awards
    that return only `gas_reward` with the existing penalty and leave the
    state as it was, recovery to full awards once the accounting is
    repaired, and an aborting store fault;
  ...
  - SWA and SRA upgrade authorization, hold, cancellation and on-chain
    ...
    scopes, same-epoch ordering of a registry write against
    settlements), reporting and correction windows, missing
    reports, removed Orchestrators (the row dropped and its share burned,
    exclusion from the quarter), `RemoveOrchestrator`
...
changes to leader election or finality.
  ...
  - **Governance over consensus.** SWA Governance can rewrite the
    ...
    hold, and the upgrade is observable on chain. One rogue multisig
    can therefore block every discretionary SWA change, by withholding
    approval or by cancelling; it cannot queue a write or cancel a
    gate step, and the remedy is the network upgrade of Section 4.3.
    f02 enforces its
  ...
  - **Governance over the registry (SRA capture).** A captured SRA
    ...
    set only shares validated to sum to one (Section 2.4) and custodies
    no balance. Misattribution is further
    ...
    report inflated volume to force a w2 step-up. The injection path
    is `CorrectVolume`: both Registry multisigs jointly replace any
    posted value during the verification window, and whatever stands
    at window close binds (Sections 2.2 and 3.2). That step is still
    ...
    against recomputation (Section 2.2). Unlike a share misallocation,
    ...
    adjustment via `SetGateParams`, sequenced so that no gate check
    applies between the two effective epochs (Section 3.1.1), so the
    counter keeps its authority over gate position (Section 4.2). The
    same window bounds the downward direction: a value corrected to
    zero binds at its close like any other, and the victim loses its
    service-stream income for about one quarter, with no
    compensation, since bound values are final for their quarter and
    an appeal affects only later quarters (Sections 2.2 and 4.3).
    Corrections bind at window close in both directions by design;
    the protections are the verification window, every value publicly
    recomputable while it runs, and the joint signature of both
    Registry multisigs, a correction having no separate hold or
    cancellation path (Section 4.3).
  ...
  - **Removal is not clawback; a hostile replace is rate-limited, not
    ...
    loss is the captured flow until that write binds; the gate reads
    the old SRA until an SWA code upgrade changes the wired SRA
    address (Section 3.1). A hold would
    not have prevented this, only delayed it: the cancel rights
    during the hold would belong to the same captured multisigs.

  - **Removing the sole Orchestrator burns the whole service stream
    until new shares take effect.** At activation the registry holds one
    Orchestrator (Section 4.3), so this is the launch configuration.
    With a single admitted Orchestrator, `RemoveOrchestrator` names
    `f099` for the map's only entry and drops it (Section 3.2), so the
    whole service stream burns from the epoch the removal binds. The
    removal excludes the Orchestrator's FPV. If the quarter's eligible
    FPV sums to zero, its `SubmitShares` is a benign no-op and the emptied
    map stands (Section 3.2). Recovery is a new admission, paid from
    `SubmitShares` for its first quarter with eligible volume (Section
    3.2). A discretionary
    write lowering w2 is not faster and does not stop the burn: it
    needs an accepted FIP and is held like any other SWA write
    (Section 4.3), and the award path burns whatever the ceiling
    leaves unallocated (Section 2.4.3).
  ...
  - **Sustainability of consensus mining.** The initial ceiling (1 −
    ...
    predictable and can be halted by a discretionary SWA write (Section
    4.2).
  ...
  - **FPV reporting: liveness and verification.** The gate’s integrity
    ...
    2.2). `CorrectVolume` needs both Registry multisigs, so one rogue
    multisig can block a correction by not signing, and the false
    posting then binds. If no figure is posted or supplied via
    `CorrectVolume`, values
    ...
    detectable afterwards: FPV is recomputable by anyone, and an
    error found after binding is answered through audit or removal,
    since bound values are final (Section 2.2).
  ...
  - **Consensus-path safety of the split.** The per-block split makes no
    ...
    with `Σ_{i>=1} w_i > 1`, and cancellation-stranded writes drop before
    ...
    party gains by inducing one. Inconsistent accounting is stricter:
    it aborts every explicit method, so no in-band write can repair it;
    awards pay only `gas_reward`, with penalties as today, and
    consensus mining runs on gas rewards alone until a network upgrade
    carrying a migration restores the state. Otherwise each allocation
    is floored and the exact residual burns, conserving `BR`
    atto-exactly.
  ...
  - **Compromised Orchestrator wallet.** An Orchestrator's payout wallet
    holds no authority over weights or other participants' shares. A key
    compromise cannot redirect the split.
    The Orchestrator's exit is `ReplaceWallet` (Section 3.2): a public
    request executed by both Registry multisigs through an immediate
    `ReplaceAddress`, so the compromised wallet stops earning the moment
    that call binds.
...
measurable path: participate and earn from real
...
which spans all affected repositories:
  ...
  - **Built-in actors:** miner-actor changes (DataCap/verified-registry
    ...
    changes in
    [builtin-actors PR #1782](https://github.com/filecoin-project/builtin-actors/pull/1782).
  - **Go state types and migration:**
    [go-state-types PR #462](https://github.com/filecoin-project/go-state-types/pull/462).
  - **Node implementations:** the
    [Lotus feature branch](https://github.com/filecoin-project/lotus/pull/13747)
    and [Forest tracking issue](https://github.com/ChainSafe/forest/issues/7552).
    Updates also cover Venus and SP tooling.
