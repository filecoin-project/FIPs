    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Parameters.* `MIN_LOT` (minimum lot face value for a
        qualifying print) and `PRICE_BAND` (maximum deviation from the
        last qualifying print's implied rate for a print to itself
        qualify; the thin-auction guard). Both are SRA state,
        governance-settable like the admitted lists (Section 3.2), and
        together fix the off-chain algorithm every indexer, and every
        verifier, must apply; a proposed initial value is maintained
        in the governance repository (`MIN_LOT` of a few hundred USD,
        sized so that correcting a mispriced qualifying lot is always
        profitable for a competing bidder). There is no cap on how
        many qualifying prints a quarter may use, and so no merging
        step: bounding and merging pricing periods only mattered when
        each period's print data had to be submitted and iterated
  ...
  - **One conversion rule, applied off-chain.** FPV is denominated in
    ...
    `PostVolume` is ever called. To recap who does what:
      ...
      - *The rule is fixed and public, so the result is
        recomputable.* Because the algorithm and its parameters
        (`MIN_LOT`, `PRICE_BAND`) are SRA state
        and every input (settlements, fee-auction claims) is a public
...
divided among Orchestrators. In particular,
  ...
  - The SRA holds the registry (the (payer, operator) to Orchestrator
    ...
    FIL pricing parameters (`MIN_LOT`, `PRICE_BAND`, Section 2.3) are
    SRA state, updated by SRA Governance under
    `SRA_CANCEL_HOLD` like any registry change (Section 4.2); the
    pricing parameters govern the off-chain conversion each
    Orchestrator's indexer applies before posting, not an on-chain
    computation. Read-only views expose the registry, the bound
...
(except the last):
  ...
  - the FIL pricing parameters (`MIN_LOT`, `PRICE_BAND`, Section 2.3);
...
Call:
  ...
  - choose the initial `MIN_LOT` and `PRICE_BAND` values for FIL
    conversion;
