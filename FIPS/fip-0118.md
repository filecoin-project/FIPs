...
`BR` atto-exactly despite per-stream floors. Gas rewards and existing
...
schedule or a new stream). w2 takes values only on the 5pp grid
...
divided among Orchestrators. In particular,
  ...
  - Its methods:
      ...
      - **`SubmitShares()`.** Permissionless. Operates on the latest
        ...
        removed in quarter Q are excluded (Section 2.2). Any `f099` entry created by a removal
      ...
      - **`ReplaceWallet(old, new)`.** Both Registry
        ...
        an immediate map push. The replace is prospective: the
        Orchestrator identity does not change and accrued balances
        are not moved. The public request and the remedy of Section
        4.3 protect against misuse; a mistaken replace is undone by
        another `ReplaceWallet`.
...
changes to leader election or finality.
  ...
  - **Removing the sole Orchestrator burns the whole service stream
    for at least a quarter.** At activation the registry holds one
    Orchestrator (Section 4.3), so this is the launch configuration.
    With a single admitted Orchestrator, `RemoveOrchestrator`
    repoints the map's only entry to `f099` (Section 3.2), so the
    whole service stream burns from the epoch the removal binds, and
    keeps burning: the removal excludes the Orchestrator's FPV, the
    ended quarter's eligible FPV sums to zero, and its `SubmitShares`
    is a benign no-op under which the existing map, now all burn
    share, stands (Section 3.2). Recovery is a new admission, paid
    from the `SubmitShares` of the quarter it is admitted in (Section
    3.2). A discretionary
