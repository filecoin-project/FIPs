    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Parameters.* `MIN_LOT` (minimum lot face value for a
        qualifying print), `PRICE_BAND` (maximum deviation from the
        last qualifying print's implied rate for a print to itself
        qualify; the thin-auction guard), and `MAX_PRICE_PERIODS`
        (upper bound on pricing periods applied per quarter; if more
        prints qualify, adjacent periods are merged and the merged
        slice is converted at the lot-weighted mean of its prints).
        All three are SRA state, governance-settable like the
        admitted lists (Section 3.2), and together fix the off-chain
        algorithm every indexer, and every verifier, must apply;
  ...
  - **One conversion rule, applied off-chain.** FPV is denominated in
    ...
    `PostVolume` is ever called. To recap who does what:
      ...
      - *The rule is fixed and public, so the result is
        ...
        (`MIN_LOT`, `PRICE_BAND`, `MAX_PRICE_PERIODS`) are SRA state
