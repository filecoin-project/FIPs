...
divided among Orchestrators. In particular,
  ...
  - It is governed by SRA Governance (Section 4): two multisig
    ...
    requiring a FIP (Section 4.2). Registry changes are not held:
    each binds at once, emits an event, and carries a mandatory
    public record of the action and its rationale in the governance
    repository. The protection a hold used to give is replaced by
    that transparency duty plus the remedy of Section 4.3: signers
    who misuse a power or dodge the duty are replaced from the level
    above. The one exception is the SRA code upgrade, which keeps
    its hold (Section 4.2).
  ...
  - The SRA holds the registry (the (payer, operator) to Orchestrator
    ...
    two Registry Safe addresses (Section 4.2); there is no queue of
    pending changes; the
  ...
  - **Removal semantics.** There is no frozen state: the registry
    knows only admitted and removed, and `RemoveOrchestrator` is the
  ...
  - Its methods:
      ...
      - **`CorrectVolume(orch, Q, value)`.** Callable only by both
        ...
        not post. The verification window is not a governance hold
        but part of the measurement cycle, and it is what bounds
      ...
      - **`SubmitShares()`.** Permissionless. Operates on the latest
        ...
        share map stands, including any `f099` entry it contains.
...
which is part of the measurement cycle rather than a governance hold: a
...
The implementation test suite MUST cover at least:
  ...
  - SWA and SRA upgrade authorization, hold, cancellation and on-chain
    ...
    accumulation, exclusion from the quarter), and identical inputs to
...
changes to leader election or finality.
  ...
  - **Removal is not clawback; a hostile replace is rate-limited, not
    ...
    fraudster. On the other side, `ReplaceWallet` is the one
