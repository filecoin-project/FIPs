...
**1.2 New sectors, existing sectors and SnapDeals.**
  ...
  - SnapDeals (`ProveReplicaUpdate3`) remain available: snapping data
    ...
    initial-pledge requirement is recomputed for the new QAP; when it
  ...
  - The pledge is raised at most once, at the first upgrade. A sector
    already at 10× is not eligible and is skipped (not a batch
    failure), so repeated calls lock nothing, even if the pledge
...
1.  **Volume reporting**. An off-chain indexer computes `FPV_i(Q)` (see
    ...
    throughout: admitted-stablecoin settlements count at face value,
    ...
    2.3 (qualifying fee-auction prints; no third-party price feed).
    `FPV_i(Q)` is therefore a single USD-denominated figure: the FIL
    contribution is folded into the same total as the stablecoin
    volume rather than posted as a separate component. Each
...
#### 2.3 FPV definitions
    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Parameters.* `PRICE_BAND` (maximum deviation from the last
        ...
        Initial values are maintained in the governance repository.
      ...
      - *Minimum lot.* `MIN_LOT` is not a constant: a threshold that
        ...
        `MIN_LOT(Q) = max(MIN_LOT_FLOOR, (MIN_LOT_ALPHA / N(Q-1)) * BoundVolume(Q-1) * FEE_RATE)`

        where `BoundVolume(Q-1)` is the previous quarter's aggregate
        bound volume (final, on-chain), `N(Q-1)` is the number of
        fee-auction claims that actually cleared on admitted Filecoin
        Pay contracts in that quarter, and `FEE_RATE` is Filecoin
        Pay's settlement fee (0.5%; fixed by that protocol, not a
        parameter of this FIP). Since `BoundVolume(Q-1) * FEE_RATE /
        N(Q-1)` is the previous quarter's average auction lot, the
        rule reads: a qualifying lot must be at least the fraction
        `MIN_LOT_ALPHA` of last quarter's average lot, never below
        `MIN_LOT_FLOOR`. In the first quarter there is no bound
        previous quarter, so `MIN_LOT(1) = MIN_LOT_FLOOR`.
  ...
  - **One conversion rule, applied off-chain.** FPV is denominated in
    USD, and the pipeline has exactly one conversion point: each
    Orchestrator's indexer, applying the FIL pricing rule above before
    `PostVolume` is ever called. To recap who does what:

      - *Orchestrators convert, then report.* Each Orchestrator's
        indexer computes `FPV_i(Q)` as a single USD total: admitted
        ...
        face value with no conversion; FIL-denominated volume is
        converted at the qualifying prints per the FIL pricing rule
        above. `PostVolume` carries only this total; the SRA never
        receives raw FIL amounts, pricing periods, or print
        references.

      - *The rule is fixed and public, so the result is
        recomputable.* Because the algorithm is fixed, its parameters
        (`MIN_LOT_FLOOR`, `MIN_LOT_ALPHA`, `PRICE_BAND`) are fixed by
        public parameter events (see *Parameters* above), and every
        input (settlements, fee-auction claims) is a public
        on-chain event, any observer, including SRA Governance during
        the verification window, can independently recompute the same
        total and compare it against what was posted (Section 2.2).
        There is no external feed to trust or to go stale; moving the
        arithmetic off-chain does not weaken this, since the
        arithmetic itself is deterministic over public inputs.

      - *The SWA reads USD only.* The gate comparison and `SplitRule`
        both consume the same posted, bound values; neither the SRA
        nor the SWA performs any conversion on-chain.
...
particular
  ...
  - The SWA holds the gate parameters (Section 3.1.1),
    ...
    position: it indexes the escalation map directly and determines
    the next w2 level.
...
divided among Orchestrators. In particular,
  ...
  - It is governed by SRA Governance (Section 4): two multisig
    accounts held by independent entities (e.g., Safes), with every
    registry change requiring the approval of both, and none
    requiring a FIP (Section 4.2). Each registry change binds at
    once, emits an event, and carries a mandatory public record of
    the action and its rationale in the governance repository. Two
    guarantees stand behind every change: the record makes it
    auditable by anyone from the moment it binds, and signers who
    misuse a power or dodge the transparency duty are replaced from
    the level above (Section 4.3). The SRA code upgrade alone waits
    in a public queue before binding (Section 4.2).

  - The SRA holds the registry (the (payer, operator) to Orchestrator
    bindings and each Orchestrator’s payout wallet), the
    ...
    last quarter whose share map was submitted, and the
    two Registry Safe addresses (Section 4.2); the
    verification-window duration is a parameter set in the
    governance repository (Section 4.4); the admitted-stablecoin
    whitelist and the admitted Filecoin Pay contract addresses are
    SRA state, updated by SRA Governance
    like any registry change and applying from the next quarter
    boundary (Section 2.3); the
    FIL pricing parameters (`MIN_LOT_FLOOR`, `MIN_LOT_ALPHA`,
    `PRICE_BAND`, Section 2.3) go through the same lifecycle but are
    not stored: their write only emits an event, since they govern
    the off-chain conversion each Orchestrator's indexer applies
    before posting, not an on-chain computation. Read-only views expose the registry, the bound
    volumes, and `AggregatedFPV(Q)` in USD (the sum of each
    Orchestrator's bound, already USD-denominated `FPV_i(Q)`, Sections
    2.2 and 2.3), which the SWA’s gate reads.
  ...
  - **Removal semantics.** The registry knows two states, admitted
    ...
    `RegisterPairs`. The removal criteria, the evidence standard,
  ...
  - Its methods:
      ...
      - **`RegisterPairs(pairs)`.** Callable by an admitted
        ...
        another Orchestrator (the uniqueness invariant, Section 2.3).
      ...
      - **`PostVolume(Q, fpv)`.** Callable by an admitted
        ...
        settlement and fee-auction events and the quarter’s registry
        state; the value binds only after the verification window
      ...
      - **`CorrectVolume(orch, Q, value)`.** Callable only by both
        ...
        not post. The verification window, part of the measurement
        cycle (Section 2.2), is what bounds
        corrections; a contested correction is
      ...
      - **`SubmitShares()`.** Permissionless. Operates on the latest
        ...
        a benign no-op: `SplitRule` is not evaluated, the existing
        share map stands, including any `f099` entry it contains, and
        the quarter still counts as submitted (so it cannot be
      ...
      - **`AddOrchestrator(orch, wallet)`.** Both Registry Safes.
        Binds at once: the Orchestrator may register pairs
        immediately. Payment lags measurement by construction: an
        Orchestrator admitted in quarter Q posts its first volume
        for quarter Q+1 and enters the share map installed about ten
        days into Q+2, when that quarter's `SubmitShares` runs.

      - **`RemoveOrchestrator(orch, reason)`.** Both Registry Safes.
        Callable only while no ended quarter awaits its share map:
        from the end of a quarter until that quarter's `SubmitShares`
        has executed (the posting period, the verification window, and
        any crank delay after it), the call reverts. Every removal
        therefore binds before the close of the posting period of the
        next quarter to settle, so the exclusion rule of Section 2.2
        applies to every removal and a removed Orchestrator never
        enters a map submitted after its removal binds. The guard
        cannot block a removal for long: `SubmitShares` is
        permissionless, so the Registry Safes clear it themselves by
        ...
        every other share is left untouched. If an `f099` entry
        already exists from an earlier removal in the same period,
        the weight is added to that entry, because f02 rejects a map
        with duplicate wallets (Section 2.4.4). Redistribution to
        the survivors is triggered only at the next `SubmitShares`,
        so nobody gains at the moment a removal binds.

      - **`ReplaceWallet(old, new, extradata)`.** Both Registry
        Safes, on a public request from the Orchestrator recorded in
        the governance repository. Swaps the payout wallet through
        an immediate map push; the Orchestrator identity does not
        move, and accrued balances stay where they accrued, a
        replace being strictly prospective. `extradata` carries the
        Orchestrator's co-signature: from the old wallet for a
        rotation (the key is still held), or from the new wallet
        when the old one is lost. The new-wallet signature is a
        liveness proof, not an authorization, since whoever proposes
        the change controls the new wallet by construction; the real
        protections are the public request and the remedy of Section
        4.3. A replace done by mistake is corrected by another
        co-signed `ReplaceWallet`.

      - **`ReassignBinding(pair, orch)`.** Both Registry Safes.
        Binds at once. Moves a contested pair: the pair's volume
        counts toward the new Orchestrator from the epoch the change
        binds, and already-posted quarters are untouched.

      - **`SetAdmittedLists(stablecoins, filecoin_pay_contracts)`.**
        Both Registry Safes. Binds at once but applies from the next
        quarter boundary, so no quarter already being measured
        changes under the Orchestrators (Section 2.3).

      - **`SetPricingParams(min_lot_floor, min_lot_alpha,
        price_band)`.** Both Registry Safes. Stores nothing: the
        call's only effect is the parameter event (Section 2.3), and
        the new values apply from the next quarter boundary, the
        same rule as the admitted lists.

      - **`ReplaceOwner(prev_owner, new_owner)`.** Both Registry
        Safes, so it cannot be used by one Safe against the other.
        Binds at once, emits an event, and carries a published
...
protection is that each one is public on chain the moment it binds and
carries a mandatory published rationale. A governance tier that fails,
misuses a power, or dodges the transparency duty is replaced from the
level above.
...
The two Safes for each contract hold its upgrade authority. Both Safes
approve every discretionary action at both tiers. At the SWA tier the
change is additionally held before it binds and either Safe alone can
cancel it during the hold; a cancellation can only preserve the status
quo, so it grants no new power. At the SRA tier registry actions bind
at once, each with an on-chain event and a published rationale; there
is no pending queue and no cancellation path (Section 3.2). A code
...
SRA's upgrade hold is the one hold the SRA itself keeps, and its
duration is a parameter set in the governance repository (Section
4.4).
...
(except the last):
  ...
  - admit or remove an Orchestrator; replace an Orchestrator's payout
    wallet on its public, co-signed request; cooperatively rotate a
    Registry Safe (`ReplaceOwner`);
...
is published and held, and either Safe can cancel during the hold;
...
at once: their protection is the mandatory public record plus the
replaceability of the tier (Sections 3.2 and 4.1), and the one held SRA
action is the code upgrade.
...
| Registry membership, wallet replacement, bindings, volume corrections, cooperative Safe rotation | Both Safes, binds at once, published rationale, no FIP |
...
An obvious simpler alternative: a single Safe as the contract's owner, itself a 2-of-2 whose two signers are each a Safe controlled by one of the two entities. Ordinary Safe co-signing already solves "two parties must agree," so why write a bespoke governance contract at all? This does not fit, for two reasons.

  - First, *co-signing and objection are different problems, and nesting Safes only solves the first one*. A multisig, nested or not, executes as soon as its threshold is met; there is no notion of a pending, published, still-reversible state afterward. With only two parties, that matters: if a bad write gets both signatures, because one signer is compromised, coerced, or simply wrong, undoing it requires assembling the same two-party consent that produced it in the first place. A corrective write and the mistake it corrects need identical authority, so "just get both to sign again" is not a safeguard against the case where one of the two is the problem. What is needed instead is a hold after approval during which either party, alone, can cancel, with no need to persuade the other. That is a distinct capability from co-signing, so it doesn't matter how the owner's signature threshold is composed; the hold and the unilateral veto have to be written as their own logic somewhere, whether that is a Safe module, a wrapper contract, or (what this FIP does) `UnanimousGovernance` ([Section 4.3](#43-the-change-lifecycle)) tracking the two owner addresses directly and granting either one a standing cancel right on any pending discretionary write.

  - Second, *lifetime and upgradeability*. SWA and SRA are long-lived and upgradeable, making continuous (weekly-to-quarterly) writes for the life of the protocol. Wherever the hold-and-veto logic lives, if it lives entirely inside upgradeable FEVM code, whether that's the governed contract or a Safe module wrapping it, it is only as trustworthy as that code's current version: a captured or maliciously upgraded SWA or SRA could rewrite or bypass its own safeguard. This is why `SWA_TIMELOCK` is additionally enforced at f02 ([Section 4.3](#43-the-change-lifecycle)): the built-in actor is not upgradeable via a contract-level code change, so the objection window survives even a fully compromised governance contract, not just a compromised signer.

  How each owner address is implemented internally, including whether it is itself a nested multisig, is unaffected by any of this ([Section 4.2](#42-governance-tiers-and-powers)): reusing Safe for co-signing was never in question, and each entity is free to do so. What a bare multisig-of-multisigs does not provide, at any nesting depth, is the post-approval hold with unilateral cancellation, the per-write cancellability distinctions between discretionary and mechanism-executed writes ([Section 4.3](#43-the-change-lifecycle)), or enforcement that survives a compromised governance contract.

  The same reasoning is why this FIP doesn't reuse [FRC-0099](../FRCs/frc-0099.md)'s [`F3Parameters`](https://github.com/filecoin-project/f3-activation-contract/blob/main/contracts/F3Parameters.sol) pattern (a single Safe-owned `Ownable` contract, used for one-time F3 activation parameters): that Safe is 3-of-3 across three independent implementations, so no single signer's error is ever sufficient to produce a bad write, and the contract is immutable and self-disabling, so there is no long-lived, upgradeable governance surface to protect. Neither property holds for SWA and SRA.
...
changes to leader election or finality.
  ...
  - **Governance over the registry (SRA capture).** A captured SRA
    ...
    registry change, public on chain the moment it binds and carrying
    a published rationale. One
  ...
  - **Removal is not clawback; a hostile replace is rate-limited, not
    ...
    profitable capture: if both Registry Safes go rogue, it can
    ...
    during the hold would belong to the same captured Safes.
  ...
  - **FPV reporting: liveness and verification.** The gate’s integrity
    ...
    bound values are final (Section 2.2). The verification-window length
    is an open parameter (Section 4.4).
  ...
  - **Compromised Orchestrator wallet.** An Orchestrator address holds
    ...
    The Orchestrator's exit is `ReplaceWallet` in its lost-key branch
    (Section 3.2): a public request co-signed from the new wallet,
    executed by both Registry Safes through an immediate map push, so
...
[Solstice project](https://github.com/orgs/filecoin-project/projects/143):
  ...
  - **FEVM contracts and tools:** current work in progress in the
...
The following items must be resolved before this FIP can enter Last
Call:
  ...
  - choose the initial `MIN_LOT_FLOOR`, `MIN_LOT_ALPHA`, and
    `PRICE_BAND` values for FIL conversion;
