...
particular
  ...
  - The SWA holds the gate parameters (Section 3.1.1),
    ...
    the next w2 level.
    It also holds the two SWA multisig addresses (Section 4.2) and
    ...
    either multisig. Its methods:
      ...
      - **`ProposeWrite(write)`.** Requires the approval of both SWA
        multisigs. Relays a discretionary write (`SetWeightRecords`,
      ...
      - **`Cancel(id, op)`.** Callable by either SWA multisig alone. Relays
        f02.`CancelPending(id, op)`, discarding a queued discretionary write
        during its timelock; a cancellation can only preserve the
        status quo and grants no new power, since either multisig could
...
divided among Orchestrators. In particular,
  ...
  - The SRA holds the registry (the (payer, operator) to Orchestrator
    ...
    two Registry multisig addresses (Section 4.2); the
  ...
  - Its methods:
      ...
      - **`RemoveOrchestrator(orch, reason)`.** Both Registry multisigs.
      ...
      - **`ReassignBinding(pair, orch)`.** Both Registry multisigs.
      ...
      - **`SetPricingParams(min_lot_floor, min_lot_alpha,
        price_band)`.** Both Registry multisigs. Stores nothing: the
...
preserve the status quo, so it grants no new power. At the SRA tier
...
| Registry membership, wallet replacement, bindings, volume corrections (bind at window close, Section 4.3), cooperative multisig rotation | Both multisigs, binds at once, published rationale, no FIP |
...
logic at all? This does not fit, for two reasons.
  ...
  - First, *co-signing and objection are different problems*: nesting
    ...
    need to persuade the other. That is a distinct capability from
    co-signing, so it does not matter how the owner's signature
    ...
    owner multisig, a wrapper contract, or (what this FIP does) the
    ...
    discretionary write (Sections 4.2 and 4.3).
  ...
  - Second, *lifetime and upgradeability*: the SWA and SRA are
    ...
    change, so the objection window survives even a fully compromised
    governance contract, not just a compromised signer.
...
changes to leader election or finality.
  ...
  - **Compromised Orchestrator wallet.** An Orchestrator address holds
    ...
    executed by both Registry multisigs through an immediate map push,
    so
