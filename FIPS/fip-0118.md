...
2.  **Verification window mechanics.**
    ...
    5.  Determinism: `FPV_i(Q)` is fully determined by (i) settlement
        ...
        removal) apply from the next quarter boundary, the same rule
        as the admitted lists (Section 3.2),
      ...
      - **`FPV_i(Q)`** = gross value settled over Filecoin Pay rails in
        ...
        operator) pairs:
          ...
          - Attribution is keyed on the (payer, operator) pair;
            ...
            A binding applies from the next quarter boundary
            (Section 2.2), so a quarter's attribution is fixed by its
            quarter-start registry snapshot.
...
divided among Orchestrators. In particular,
  ...
  - **Removal semantics.** The registry knows two states, admitted
    ...
    `RegisterPairs`; a re-registration, like any binding change,
    applies from the next quarter boundary (Section 2.2). The
  ...
  - Its methods:
      ...
      - **`RegisterPairs(pairs)`.** Callable by an admitted
        ...
        A registration binds at once, so the uniqueness invariant is
        enforced at the call, but applies from the next quarter
        boundary: the pair's volume counts toward the Orchestrator
        from the first epoch of the next quarter (Section 2.2).
      ...
      - **`AddOrchestrator(orch, wallet)`.** Both Registry Safes.
        ...
        immediately, and each registration applies from the next
        quarter boundary (Section 2.2). Payment therefore lags
      ...
      - **`ReplaceWallet(old, new, extradata)`.** Both Registry
        ...
        balances stay where they accrued. `extradata` carries the
      ...
      - **`ReassignBinding(pair, orch)`.** Both Registry Safes.
        Binds at once but applies from the next quarter boundary,
        like every binding change (Section 2.2). Moves a contested
        pair: its volume counts toward the new Orchestrator from the
        first epoch of the next quarter, the quarter in which the
        change binds is still measured on its quarter-start snapshot,
        and already-posted quarters are untouched.
...
The implementation test suite MUST cover at least:
  ...
  - SWA and SRA upgrade authorization, hold, cancellation and on-chain
    ...
    pricing-print boundaries, binding changes applying only from the
    next quarter boundary, reporting and correction windows, missing
...
changes to leader election or finality.
  ...
  - **Governance over the registry (SRA capture).** A captured SRA
    ...
    published rationale, and applying only from the next quarter
    boundary (Section 2.2). One
