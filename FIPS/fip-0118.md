    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Minimum lot.* `MIN_LOT` is not a constant: a threshold that
        ...
        fixed by that protocol, not a parameter of this FIP).
        `BoundVolume(Q-1) * FEE_RATE / N(Q-1)` spreads the previous
        quarter's settlement fees on bound volume evenly over that
        quarter's cleared claims: a scale reference for lot sizes,
        not the realized average lot. Filecoin Pay burns the fees on
        FIL settlements instead of auctioning them (FIL pricing rule
        above), so a growing FIL share of bound volume lifts the
        reference above the lots the auctions actually clear;
        lowering `MIN_LOT_ALPHA` compensates. The rule reads: a
        qualifying lot must be at least the fraction `MIN_LOT_ALPHA`
        of this reference, never below `MIN_LOT_FLOOR`.
