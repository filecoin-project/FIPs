...
**1.2 New sectors, existing sectors and SnapDeals.**
  ...
  - The pledge is raised at most once, at the first upgrade, in either
    ...
    10× is not eligible and is skipped (not a batch failure), so
    repeated calls lock nothing, even if the pledge requirement has
    risen since. A 10× sector that only needs an extension uses
    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Minimum lot.* `MIN_LOT` is not a constant: a threshold that
        ...
        `MIN_LOT(Q) = max(MIN_LOT_FLOOR, floor(MIN_LOT_ALPHA * BoundVolume(Q-1) * FEE_RATE / N(Q-1)))`
        ...
        Pay's settlement fee, exactly the rational `1/200` (0.5%;
        fixed by that protocol, not a parameter of this FIP). Since
        `BoundVolume(Q-1) * FEE_RATE / N(Q-1)` is the previous
        quarter's average auction lot, the rule reads: a qualifying
        lot must be at least the fraction `MIN_LOT_ALPHA` of last
        quarter's average lot, never below `MIN_LOT_FLOOR`.
        ...
        way: `N(Q-1) = 0` gives no average lot, and `MIN_LOT(Q) =
        MIN_LOT_FLOOR`.
...
`floor(share * accrued[id] / DENOM)` minus what it has already claimed
