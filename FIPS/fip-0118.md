...
Recap of the values the FIL pricing rule depends on:
...
| `MIN_LOT_FLOOR` | Input to `MIN_LOT(Q)` | Governance parameter (atto-USD) | Parameter events | $0.50 (5 * 10^17 atto-USD) | Next quarter boundary | The fallback whenever the reference is unavailable |
| `MIN_LOT_ALPHA` | Input to `MIN_LOT(Q)` | Governance parameter (rational: numerator and denominator) | Parameter events | 1/400 | Next quarter boundary | Retuned as the relation between bound volume and lot sizes shifts |
...
| Admitted lists | Scope prints and FPV: a print is a claim on an admitted contract, of an admitted stablecoin | SRA registry state | `SetAdmittedLists` events | Installed pre-activation (Section 2.2) | Next quarter boundary | None |
