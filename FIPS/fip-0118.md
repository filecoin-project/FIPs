...
2.  **Verification window mechanics.**
    ...
    5.  Determinism: `FPV_i(Q)` is fully determined by (i) settlement
        ...
        fixed at rail creation, (iii) the registry bindings in force
        at quarter Q's first epoch, (iv) gross settled value in the
        ...
        admitted contracts.
        Binding changes (registration, reassignment, release by
        removal) take measurement effect from the next quarter
        boundary, the same rule as the admitted lists (Section 3.2),
        so within any one quarter the registry, like the admitted
        lists, is constant: a verifier needs only the quarter-start
        registry snapshot, never per-settlement binding epochs.
      ...
      - **`FPV_i(Q)`** = gross value settled over Filecoin Pay rails in
        ...
        operator) pairs:
          ...
          - Attribution is keyed on the (payer, operator) pair;
            ...
            A binding takes measurement effect from the next quarter
            boundary (Section 2.2), so a quarter's attribution is
            fixed by its quarter-start registry snapshot.
...
divided among Orchestrators. In particular,
  ...
  - **Removal semantics.** The registry knows two states, admitted
    ...
    `RegisterPairs`, a re-registration taking measurement effect from
    the next quarter boundary like any binding change (Section
    2.2). The removal criteria, the evidence standard,
  ...
  - Its methods:
      ...
      - **`RegisterPairs(pairs)`.** Callable by an admitted
        ...
        A pair binds at once for the uniqueness invariant but takes
        measurement effect from the next quarter boundary: its volume
        counts from the first epoch of the quarter after registration
        (Section 2.2).
      ...
      - **`AddOrchestrator(orch, wallet)`.** Both Registry Safes.
        ...
        immediately, and those pairs take measurement effect at the
        next quarter boundary (Section 2.2). Payment therefore lags
        measurement by construction: an
      ...
      - **`ReassignBinding(pair, orch)`.** Both Registry Safes.
        Binds at once but takes measurement effect from the next
        quarter boundary, like every binding change (Section 2.2).
        Moves a contested pair: its volume counts toward the new
        Orchestrator from the first epoch of the next quarter; the
        quarter in which the change binds is still measured on its
        quarter-start snapshot, and already-posted quarters are
        untouched.
...
The implementation test suite MUST cover at least:
  ...
  - SWA and SRA upgrade authorization, hold, cancellation and on-chain
    ...
    pricing-print boundaries, binding changes taking measurement
    effect only at the next quarter boundary, reporting and correction
    windows, missing reports, removed Orchestrators (immediate burn, `f099`
...
changes to leader election or finality.
  ...
  - **Governance over the registry (SRA capture).** A captured SRA
    ...
    registry change, public on chain the moment it binds, carrying a
    published rationale, and taking measurement effect only from the
    next quarter boundary (Section 2.2). One
