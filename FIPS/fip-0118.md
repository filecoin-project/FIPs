...
*Figure 2: one quarter, end to end. Quarter Q covers the epochs from `End(Q-1)` up to, not including, `End(Q) = ACTIVATION_EPOCH + Q * EPOCHS_PER_QUARTER`, so `End(Q)` is the first epoch of quarter Q+1 and quarter 1 starts at `ACTIVATION_EPOCH`; the figure's numbered steps 1 to 6 all run after `End(Q)`. Durations shown (3-day posting, 7-day verification) are the initial mainnet values, fixed at SRA deployment (Section 3.2); `SWA_TIMELOCK` is 7 days (Section 2.4). `QuarterlyGateCheck` (4) and `SubmitShares` (5) are independent and can run in either order. FIL-to-USD conversion (Section 2.3) happens off-chain, before posting.*
...
2.  **Verification window mechanics.**
    ...
    1.  Let quarter Q end at epoch `End`. Posting: each Orchestrator posts
        `FPV_i(Q)` to the SRA during the posting period (`End`, `End` +
        `POST_PERIOD`].

    2.  Verification: the verification window is a fixed phase of the
        quarter timeline, (`End` + `POST_PERIOD`, `End` + `POST_PERIOD` +
    ...
    5.  Determinism: `FPV_i(Q)` is fully determined by (i) settlement
        ...
        **Application rules.** An Orchestrator claims a payer by
        ...
        less than `REGISTRATION_CUTOFF` before `End(Q)` applies from
        `End(Q)` and covers nothing of quarter Q, so no claim can
        ...
        admitted lists and the pricing parameters apply from the end
        of the quarter in which the write executes, so no quarter
        already being measured changes scope under the Orchestrators
        (Section 3.2). Within an epoch, registry writes and
        settlements follow the total order of *Print ordering*
        (Section 2.3): a write applies to the settlements strictly
        after it in execution order, and "epoch" in these rules names
        that position. A verifier reconstructs the binding timeline
        ...
        Recap of the application rules above, as the indexer applies
        them when it attributes a pair's settlements to an `FPV_i(Q)`
        (the text governs; the table only collects it):

        | Registry write | Caller | Application epoch | The pair's settlements before the application epoch count for | The pair's settlements from the application epoch count for |
        | --- | --- | --- | --- | --- |
        ...
        | `RegisterPairs` less than `REGISTRATION_CUTOFF` before `End(Q)` | Same | `End(Q)` | Nobody: nothing of quarter Q | The registrant |
...
**Payment period.** The days a quarter's numbers pay are a different set of days than the days the quarter measured. Quarter Q's gate write (step 4) queues in f02 under `SWA_TIMELOCK`, so the stepped w2 pays no earlier than `End(Q) + POST_PERIOD + VERIFICATION_WINDOW + SWA_TIMELOCK`, about 17 days into quarter Q+1 with the initial values (Section 3.2); the share map (step 5) binds without a hold, from about 10 days in. Both offsets are floors, not schedules: `QuarterlyGateCheck` and `SubmitShares` are permissionless cranks, nothing on chain executes them, and the timelock clock starts at the gate check's execution epoch, so each effect arrives exactly as late as its crank. A late crank moves an effect later, never earlier, and reads the same bound values (step 3), so delay changes when quarter Q's numbers pay, never what they pay. Meanwhile the gap burns: f02 evaluates the burn residual w0 every epoch (Section 2.1), so each day of offset, and each day of crank delay, burns the difference between the ceiling and the standing w2; Parameter Choices and Calibration quantifies this baseline.
...
#### 2.3 FPV definitions
    ...
    *reading* only: FIL settlements themselves never go through an
    ...
    regardless of who computes it.
      ...
      - *Parameters.* `PRICE_BAND` (maximum deviation from the last
        ...
        applies from the end of the quarter in which it executes, the
        same rule as the
  ...
  - **Filecoin Pay contract versions.** "Filecoin Pay" refers to the
    ...
    required. A list update applies from the end of the quarter in
    which it executes
...
| --- | --- | --- | --- | --- | --- | --- |
| `PRICE_BAND` | Thin-auction guard: maximum deviation of a print's rate from the band reference | Governance parameter | Parameter events | 30% | End of the quarter in which it executes | The first prints have no reference: *Band seeding* above |
...
| `MIN_LOT_FLOOR` | Input to `MIN_LOT(Q)` | Governance parameter (atto-USD) | Parameter events | $0.50 (5 * 10^17 atto-USD) | End of the quarter in which it executes | None |
| `MIN_LOT_ALPHA` | Input to `MIN_LOT(Q)` | Governance parameter (rational: numerator and denominator) | Parameter events | 1/400 | End of the quarter in which it executes | Retuned when needed |
...
| Admitted lists | Scope prints and FPV: a print is a claim on an admitted contract, of an admitted stablecoin | Governance parameter, event-only | `SetAdmittedLists` events | Installed pre-activation (Section 2.2) | End of the quarter in which it executes | None |
...
| `REGISTRATION_CUTOFF` | Late-claim guard: a registration in the last cutoff of a quarter applies from `End(Q)` (Section 2.2) | Governance parameter | Parameter events | 20,160 epochs (7 days) | End of the quarter in which it executes | None |
...
w2 hits its cap exactly at `End(Q1)`, exiting the bootstrap ramp
...
reaches `W2_BASE` (10%) exactly at `End(Q1)`, where the
...
automatically from `End(Q1)`: w1 keeps ramping down while w2
...
divided among Orchestrators. In particular,
  ...
  - **State.** The SRA stores:
    ...
    Governance like any registry change, applying from the end of the
    quarter in which the write executes
  ...
  - Its methods:
      ...
      - **`RegisterPairs(pairs)`.** Callable by an admitted
        ...
        2.3).
        A registration binds at once and applies from the execution epoch,
        ...
        `REGISTRATION_CUTOFF` before `End(Q)` applies from
        `End(Q)` instead, so a claim near the close leaves
      ...
      - **`AddOrchestrator(orch, wallet)`.** Both Registry multisigs.
        ...
        registrations applying under the rules of Section 2.2
        (back-cover to the quarter's first epoch, the cutoff). An
      ...
      - **`ReassignBinding(pair, orch, inherit)`.** Both Registry
        ...
        registration's execution epoch: the new binding's application is
        computed from that epoch under the application rules of
        Section 2.2 (back-cover, the cutoff), so the rightful holder
        gets exactly the span it
      ...
      - **`CancelBinding(pair)`.** Callable by the bound Orchestrator,
        ...
        deleted and the release applies at the execution epoch, like a
        release by removal (Section 2.2), so the Orchestrator keeps
        the span it served and any admitted Orchestrator may claim
        the pair again through `RegisterPairs`, with back-cover
        reaching no further back than the release. This is the
        cooperative exit, a client relationship that ended or a
        client moving to another Orchestrator; a contested binding is
        governance's, through `ReassignBinding` or removal. The event
      ...
      - **`SetAdmittedLists(stablecoins, filecoin_pay_contracts)`.**
        ...
        end of the quarter in which it executes, so no quarter already
        being measured
      ...
      - **`SetPricingParams(min_lot_floor, min_lot_alpha,
        ...
        the end of the quarter in which it executes, the same rule as
        the admitted
...
The implementation test suite MUST cover at least:
  ...
  - SWA and SRA upgrade authorization, hold, cancellation and on-chain
    ...
    scopes), reporting and correction windows, missing
...
changes to leader election or finality.
  ...
  - **Removing the sole Orchestrator burns the whole service stream
    ...
    burn stands: the removal excludes the Orchestrator's FPV, the
    ...
    share, stands (Section 3.2). Recovery is a new admission, and
    payment still lags measurement: a successor admitted in the
    removal's quarter Q posts `FPV_i(Q)` in Q's posting period and
    enters the share map about ten days into Q+1, through that
    quarter's `SubmitShares`, never a mid-quarter push (Sections 3.2
    and 2.4.4). No lever is faster, and none
    stops the burn: a discretionary write lowering w2 needs an
    accepted FIP and is held like any other SWA write (Section 4.3),
    and it relocates rather than stops the burn, since the award
    path burns whatever the ceiling leaves unallocated (Section
    2.4.3). This is by design.
