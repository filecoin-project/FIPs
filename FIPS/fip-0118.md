...
the rejection can occur only after a discretionary write, applied or
pending, has taken part of the ceiling `1 − w1`: a change to w1's
schedule or a new stream). A conflicting pending write blocks the gate
even if it takes effect later: admission cannot invalidate a queued
write (Section 2.4.7). w2 takes values only on the 5pp grid
