...
- Constraints:
  - The `ProvingDeadline` must be between 0 and 47, inclusive. It corresponds to the Window PoST deadline index at which all the new sectors must be regularly proven. The `ProvingDeadline` must not be the current or next deadline to be proven by the receiving actor. If an illegal deadline is submitted, the message will fail with an error code `ErrIllegalArgument` and the error message:
    - "ProvingDeadline submitted X, must be between 0 and 47, and can't be current deadline Y or next deadline Z"
