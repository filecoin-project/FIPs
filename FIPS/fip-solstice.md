...
# FIP-XXXX: (Solstice) Deprecate FIL+ and Fund Services via Block-Reward Split
...
1.  **Deprecate datacap minting and the FIL+ allocation system** and
    ...
    immediately at onboarding regardless of content. The initial
...
is not yet proven, the freed share is burned, reducing dilution for all
...
allocations, no claims, no claim-term extensions. Complete removal is
...
3.  **Service Rewards Actor (SRA): the split AMONG orchestrators.** It
    computes the orchestrator shares s_i and holds the registry
...
a Distribution (and, when EXPLICIT, naming its single writer). f02 never
computes a distribution; the writer contract computes the shares off the
per-epoch path and pushes the map through SetShares, exactly as the SRA
does for the service stream, so a new stream needs no new L1 logic. Note
that f02 stores only the writer's address and the current map, never the
rules: how the shares are computed (for the service stream, SplitRule
over FPV volumes) lives inside the writer contract, which carries its
own governance (the SRA under SRA Governance, Section 4).
...
9)  **SetDistribution**(id, distribution) re-points a stream’s
    ...
    IMPLICIT is not permitted (IMPLICIT is consensus-only).
