...
Filecoin's Expected Consensus (EC) comes with probabilistic finality, and a 900-epoch soft finality threshold. While network participants (e.g. exchanges, L2 operators, application developers) use different confirmation thresholds, they pessimistically wait between 100-900 epochs before considering a transaction final, leading to delays in the order of hours. Instead of naively counting the number of epochs, we propose a confirmation rule that takes into account what takes place during those epochs and, under expected operating conditions, can attain the same level of certainty in fewer epochs. For example, consider two cases: in case A, the chain grew by 5 blocks per epoch over the last 50 epochs; in case B, the chain grew by a mere 2 blocks per epoch over the same 50 epochs. The probability of a chain reconfiguration longer than 50 epochs is much smaller in case A than in case B, thereby allowing us to make a faster finality determination.
...
* The assumption that a block $B_h$ that is produced by an honest participant (validator/SP) at epoch $r$ is visible to all other honest participants by the start of epoch $r+1$, and consequently, the branch that $B_h$ is building upon is also visible to the other participants by epoch $r+1$. This assumption is slightly weaker than the synchrony assumption typically used in Filecoin.
  
  The analysis relies on the Consistent Broadcast mechanism that was proposed in [FRC-0051](https://github.com/filecoin-project/FIPs/blob/master/FRCs/frc-0051.md) and subsequently implemented in lotus, and is therefore in use by most Filecoin nodes.

* The analysis of adversarial possibilities for different eras, with specific random variables assigned to each one:
  1. **Distant past**: adversarial lead building, denoted $L_f$
  2. **Recent past**: competing chain construction, denoted $B_f$
  3. **Future**: adversarial overtaking, denoted $M_f$
  
  Each era has different characteristics that need to be addressed. Afterwards, the results are combined to get an upper bound on the probability of error. The images below illustrate the different eras.

![Lf](../resources/frc-xxxx/Lf.png "Lf")

![Bf](../resources/frc-xxxx/Bf.png "Bf")

![Mf](../resources/frc-xxxx/Mf.png "Mf")

TODO: Not sure the images are understandable/helpful
...
![Good times](../resources/frc-xxxx/err_prob_and_block_count_on_good_times.png "Good times")
...
![Bad times](../resources/frc-xxxx/err_prob_and_block_count_on_bad_times.png "Bad times")
...
A prototype Python implementation of the algorithm is available [here](https://github.com/guy-goren/ECFinalityCalculator/blob/master/validator_calc_finality.py). This implementation has not been optimized for performance. As an application-side optimization with no interoperability requirement, implementers in different clients and languages may freely diverge from the prototype. 
...
2. Edge cases. Adding a constant as an input representing the maximal _observed_ competing chain.
