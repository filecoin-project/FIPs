...
fn transfer_from({from: Address, to: Address, amount: TokenAmount, operatorData: Bytes}) 
...
### Interface FRC0046TokenReceiver
...
// The tokenData is arbitrary data originating from the token actor.
// The operatorData is the data value provided by the actor initiating the transfer.
// Amount must be non-negative (but may be zero).
// Aborts if the receiver refuses the transfer.
fn token_received({from: Address, to: Address, operator: Address, amount: TokenAmount, operatorData: Bytes, tokenData: Bytes})
```
...
fn asset_received({type: String, data: Bytes, operatorData: Bytes})
```
